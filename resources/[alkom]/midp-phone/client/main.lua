...
RegisterNetEvent('midp-phone:LoadHP')
AddEventHandler('midp-phone:LoadHP', function()
    LoadPhone()
    exports['midp-tasknotify']:SendAlert('success', '[DEBUG] PHONE LOADED')
end)
...
RegisterNetEvent('midp-phone:client:RaceNotify', function(message)
...
RegisterNetEvent('midp-phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('midp-phone:server:SetPhoneAlerts', 'phone')
...
RegisterNUICallback('getdocsme', function(data)
    TriggerServerEvent('midp-phone:server:savedocuments', data)
end)

RegisterNUICallback('GetNote_for_Documents_app', function(data, cb)
    lib.callback('midp-phone:server:GetNote_for_Documents_app', false, function(Has)
        cb(Has)
    end)
end)

RegisterNetEvent('midp-phone:getdocuments', function()
    SendNUIMessage({
...
RegisterNUICallback('ClearRecentAlerts', function(data, cb)
    TriggerServerEvent('midp-phone:server:SetPhoneAlerts', 'phone', 0)
...
RegisterNUICallback('SetBackground', function(data)
    ...
    TriggerServerEvent('midp-phone:server:SaveMetaData', 'background', background)
end)
...
RegisterNetEvent('midp-phone:openPhone', function()
    OpenPhone()
end)

RegisterCommand('phone', function()  
    TriggerEvent('midp-phone:openPhone') 
end)
...
CreateThread(function()
    while true do
        Wait(60000)
        if isLoggedIn then
            lib.callback('midp-phone:server:GetPhoneData', false, function(pData)
...
function OpenPhone()
    ...
    if phone > 0 then
        lib.callback('midp-phone:server:GetCharacterData', false, function(chardata)
            PlayerJob = ESX.PlayerData.job
            PhoneData.PlayerData = ESX.PlayerData
            PhoneData.Twitter = {}
            ...
            -- Garage Fix
            lib.callback('midp-phone:server:GetGarageVehicles', false, function(vehicles)
            ...
            lib.callback('midp-phone:server:getBackground', false, function(backp)
                if backp ~= nil then
            ...
            lib.callback('midp-phone:server:getAvatar', false, function(avtr)
                if avtr ~= nil then
        ...
        exports['midp-tasknotify']:SendAlert('error', 'Anda Tidak Memiliki HP!')
    end
end
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('midp-phone:server:RemoveMail', MailId)
    cb('ok')
end)

RegisterNetEvent('midp-phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = 'UpdateMails',
        Mails = NewMails
    })
...
RegisterNUICallback('AcceptMailButton', function(data)
    ...
    TriggerEvent(data.buttonEvent, data.buttonData)
    TriggerServerEvent('midp-phone:server:ClearButtonData', data.mailId)
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('midp-phone:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    lib.callback('midp-phone:server:GetPicture', false, function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNetEvent("midp-phone:client:BankNotify", function(text)
    SendNUIMessage({
        action = "PhoneNotification",
        NotifyData = {
            title = "Bank",
...
RegisterNUICallback('GetBankData', function(data, cb)
    lib.callback('midp-phone:server:GetBankData', false, function(data)
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('midp-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('midp-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    lib.callback('midp-phone:server:GetContactPicture', false, function(Chat)
        SendNUIMessage({
            action = 'UpdateChat',
            chatData = Chat,
...
RegisterNetEvent('midp-phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    ...
    if New then
        ...
        if PhoneData.isOpen then
            ...
            Wait(100)
            lib.callback('midp-phone:server:GetContactPictures', false, function(Chats)
                SendNUIMessage({
                    action = "UpdateChat",
	    ...
	    SendNUIMessage({
            ...
            TriggerServerEvent('midp-phone:server:SetPhoneAlerts', "whatsapp")
        end
    else
...
RegisterNetEvent('midp-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
            title = "Mail",
    ...
    TriggerServerEvent('midp-phone:server:SetPhoneAlerts', 'mail')
end)

RegisterNUICallback('PostAdvert', function(data, cb)
    TriggerServerEvent('midp-phone:server:AddAdvert', data.message, data.url)
    cb('ok')
end)

RegisterNUICallback("DeleteAdvert", function(_, cb)
    TriggerServerEvent("midp-phone:server:DeleteAdvert")
    cb('ok')
end)

RegisterNetEvent('midp-phone:client:UpdateAdvertsDel', function(Adverts)
...
RegisterNetEvent('midp-phone:client:UpdateAdverts', function(Adverts, LastAd)
    PhoneData.Adverts = Adverts
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('midp-phone:server:SetPhoneAlerts', 'whatsapp', newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    lib.callback('midp-phone:server:CanPayInvoice', false, function(CanPay)
        if CanPay then
...
function PayInvoice(cb,invoiceId)
    ...
    ESX.TriggerServerCallback('esx_billing:payBill', function()
        lib.callback('midp-phone:server:GetInvoices', false, function(Invoices)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    ESX.TriggerServerCallback('midp-phone:server:DeclineInvoice', function(CanPay, Invoices)
        PhoneData.Invoices = Invoices
        cb('ok')
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('midp-phone:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)

RegisterNetEvent('midp-phone:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNUICallback('DeleteTweet',function(data, cb)
    TriggerServerEvent('midp-phone:server:DeleteTweet', data.id)
    cb('ok')
end)
...
RegisterNUICallback('UpdateProfilePicture', function(data)
    ...
    TriggerServerEvent('midp-phone:server:SaveMetaData', 'profilepicture', pf)
end)

RegisterNetEvent('midp-phone:updateForEveryone', function(newTweet)
    PhoneData.Tweets = newTweet
end)

RegisterNetEvent('midp-phone:updateidForEveryone', function()
...
RegisterNUICallback('LoginTwitter', function(data, cb)
    lib.callback('midp-phone:twitterLogin', false, function(success)
        if success then
...
RegisterNUICallback('RegisterTwitter', function(data, cb)
    ...
    lib.callback('midp-phone:twitterRegister', false, function(success)
        if success then
            SendNUIMessage({
                action = 'PhoneNotification',
                PhoneNotify = {
    ...
    if #Hashtag <= 3 then
        for i = 2, #Hashtag, 1 do
            ...
            if Handle ~= nil or Handle ~= "" then
                ...
                TriggerServerEvent('midp-phone:server:UpdateHashtags', Handle, TweetMessage)
            end
        end
        ...
        for i = 2, #MentionTag, 1 do
            ...
            if Handle ~= nil or Handle ~= "" then
                if Handle ~= PhoneData.Twitter.username then
                    TriggerServerEvent('midp-phone:server:MentionedPlayer', Handle, TweetMessage)
                end
            end
        end
        ...
        TriggerServerEvent('midp-phone:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
    else
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNUICallback("TakePhoto", function(_,cb)
    ...
    while takePhoto do
        ...
        elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
            lib.callback("midp-phone:server:GetWebhook", false, function(hook)
                if hook then
                    exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(data)
                        local image = json.decode(data)
                        DestroyMobilePhone()
                        CellCamActivate(false, false)
                        TriggerServerEvent('midp-phone:server:addImageToGallery', image.attachments[1].proxy_url)
                        Wait(400)
                        TriggerServerEvent('midp-phone:server:getImageFromGallery')
...
RegisterNetEvent('midp-phone:client:TransferMoney', function(amount, newmoney)
    if PhoneData.isOpen then
        SendNUIMessage({ action = 'UpdateBank', NewBalance = newmoney })
    end
end)

RegisterNetEvent('midp-phone:client:UpdateTweets', function(src, Tweets, NewTweetData, delete)
...
RegisterNetEvent('midp-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
...
RegisterNetEvent('midp-phone:refreshImages', function(images)
...
RegisterNUICallback('ClearMentions', function()
    ...
    TriggerServerEvent('midp-phone:server:SetPhoneAlerts', 'twitter', 0)
    SendNUIMessage({ action = 'RefreshAppAlerts', AppData = Config.PhoneApplications })
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('midp-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = 'RefreshAppAlerts', AppData = Config.PhoneApplications })
    end)
end)

RegisterNetEvent("midp-phone:client:CustomNotification", function(title, text, icon, color, timeout) -- Send a PhoneNotification to the phone from anywhere
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
            title = title,
...
RegisterNUICallback('TransferMoney', function(data, callback)
    ...
    lib.callback('midp-phone:server:GetBankData', false, function(data)
        ...
        if tonumber(bank) >= amount then
            ...
            TriggerServerEvent('midp-phone:server:TransferMoney', data.iban, amount)
            local cbdata = {
                CanTransfer = true,
...
RegisterNUICallback('CallContact', function(data, cb)
    lib.callback('midp-phone:server:GetCallState', false, function(CallState)
...
function CancelCall()
    TriggerServerEvent('midp-phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('midp-phone:server:SetCallState', false)
...
function CallContact(CallData, AnonymousCall)
    ...
    TriggerServerEvent('midp-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('midp-phone:server:SetCallState', true)
...
function AnswerCall()
    if (PhoneData.CallData.CallType == 'incoming' or PhoneData.CallData.CallType == 'outgoing') and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('midp-phone:server:AnswerCall', PhoneData.CallData)
...
RegisterNetEvent('midp-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == 'ongoing' then
        SendNUIMessage({
...
RegisterNetEvent('midp-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('midp-phone:server:AddRecentCall', 'missed', CallData)
                    break
                end
                Wait(Config.RepeatTimeout)
            else
...
RegisterNetEvent('midp-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == 'incoming' or PhoneData.CallData.CallType == 'outgoing') and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = 'ongoing'
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    lib.callback('midp-phone:server:FetchResult', false, function(result)
        cb(result)
    end, data.input)
end)

RegisterNUICallback('FetchVehicleResults', function(data, cb)
    lib.callback('midp-phone:server:GetVehicleSearchResults', false, function(result)
        if result ~= nil then
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    ...
    lib.callback('midp-phone:server:ScanPlate', false, function(result)
...
RegisterNetEvent('midp-phone:client:addPoliceAlert')
AddEventHandler('midp-phone:client:addPoliceAlert', function(alertData, tujuan)
    if PlayerJob.name == tujuan then
        ...
        TriggerServerEvent('midp-phone:server:SetPhoneAlerts', "meos", Config.PhoneApplications["meos"].Alerts)
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    exports['midp-tasknotify']:SendAlert('inform', 'GPS Set: '..data.alert.title)
...
RegisterNetEvent('midp-phone:client:GiveContactDetails', function()
    local ped = PlayerPedId()

    local player, distance = ESX.Game.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('midp-phone:server:GiveContactDetails', PlayerId)
    else
        exports['midp-tasknotify']:SendAlert('error', 'Tidak Ada Orang Di Sekitar')
    end
end)
...
RegisterNUICallback('DeleteImage', function(image, cb)
    TriggerServerEvent('midp-phone:server:RemoveImageFromGallery',image)
    Wait(400)
    TriggerServerEvent('midp-phone:server:getImageFromGallery')
    cb(true)
end)
...
RegisterNUICallback('track-vehicle', function(data, cb)
    ...
    if findVehFromPlateAndLocate(veh.plate) then
        exports['midp-tasknotify']:SendAlert('success', 'Tracking Berhasil')
    else
        exports['midp-tasknotify']:SendAlert('error', 'Gagal Ditemukan')
    end
    cb("ok")
end)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('midp-phone:server:RemoveContact', Name, Number)
end)

RegisterNetEvent('midp-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications['phone'].Alerts = Config.PhoneApplications['phone'].Alerts + 1
    TriggerServerEvent('midp-phone:server:SetPhoneAlerts', 'phone', Config.PhoneApplications['phone'].Alerts)
end)

RegisterNetEvent('midp-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNetEvent('midp-phone:client:NotiftfRWT', function()
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
            title = "Bank",
...
RegisterNetEvent('midp-phone:client:UpdateLapraces', function()
    SendNUIMessage({
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    ESX.TriggerServerCallback('laprace:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            exports['midp-tasknotify']:SendAlert('success', 'You are too far from the race. Your navigation is set to the race.')
...
RegisterNUICallback('GetPesanJobs', function(data, cb)
    ...
    if input then
        local pesan = input[1]
        TriggerServerEvent('midp-phone:addsms', coords, pesan, data.ContactData.number, time)
    end
end)
...
RegisterNUICallback('GetPlayerHouses', function(_, cb)
    ESX.TriggerServerCallback('midp-phone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)
]]
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    exports['midp-tasknotify']:SendAlert('success', 'Set GPS: ' .. data.HouseData.HouseData.adress)
    cb("ok")
end)

--[[
RegisterNUICallback('GetPlayerKeys', function(_, cb)
    ESX.TriggerServerCallback('midp-phone:server:GetHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    ESX.TriggerServerCallback('midp-phone:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
...
RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    ESX.TriggerServerCallback('midp-phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
]]
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    exports['midp-tasknotify']:SendAlert('success', 'Set GPS')
end)
...
RegisterNUICallback('GetCurrentLawyers', function(data, cb)
    lib.callback('midp-phone:server:GetCurrentLawyers', false, function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterCommand('testem', function()
    TriggerServerEvent('midp-phone:server:sendNewMail', { sender = 'B4RKSY.GG', subject = 'Test!', message = 'https://discord.gg/dailyliferoleplay'})
end)
]]

RegisterCommand('fixphone', function()
    LoadPhone()
    exports['midp-tasknotify']:SendAlert('success', 'Phone fixed')
end)