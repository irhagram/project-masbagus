...
-- Twitter --
RegisterNetEvent('midp-phone:server:DeleteTweet', function(tweetId)
    ...
    if delete then
        ...
        TriggerClientEvent('midp-phone:client:UpdateTweets', -1, TWData, nil, true)
    end
end)

RegisterNetEvent('midp-phone:server:UpdateTweets', function(NewTweets, TweetData)
    local src = source
    ...
    TriggerClientEvent('midp-phone:client:UpdateTweets', -1, src, NewTweets, TweetData, false)
end)

RegisterNetEvent('midp-phone:server:MentionedPlayer', function(username, TweetMessage)
	...
	for _, Player in pairs(xPlayers) do
        ...
        if Player ~= nil then
            if (character[1] and character[1].username == username) then
                ...
                TriggerClientEvent('midp-phone:client:GetMentioned', Player.source, TweetMessage, AppAlerts[Player.identifier]["twitter"])
            else
...
RegisterNetEvent('midp-phone:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('midp-phone:client:UpdateHashtags', -1, Handle, messageData)
end)

lib.callback.register('midp-phone:twitterLogin', function(source, username, password)
...
lib.callback.register('midp-phone:twitterRegister', function(source, username, password)
    local Player = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('midp-phone:server:AddAdvert', function(msg, url)
    local src = source
    ...
    TriggerClientEvent('midp-phone:client:UpdateAdverts', -1, Adverts, "@" .. result[1].firstname .. "" .. result[1].lastname)
end)

RegisterNetEvent('midp-phone:server:DeleteAdvert', function()
    local Player = ESX.GetPlayerFromId(source)
    local Identifier = Player.identifier
    Adverts[Identifier] = nil
    
    TriggerClientEvent('midp-phone:client:UpdateAdvertsDel', -1, Adverts)
end)
...
RegisterNetEvent('midp-phone:server:updateForEveryone', function(newTweet)
    local src = source
    TriggerClientEvent('midp-phone:updateForEveryone', -1, newTweet)
end)

RegisterNetEvent('midp-phone:server:updateidForEveryone', function()
    TriggerClientEvent('midp-phone:updateidForEveryone', -1)
end)

lib.callback.register('midp-phone:server:GetPhoneData', function(source)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
lib.callback.register('midp-phone:server:GetCallState', function(source, ContactData)
...
RegisterNetEvent('midp-phone:server:SetCallState', function(bool)
    local src = source
...
RegisterNetEvent('midp-phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    ...
    SetTimeout(100, function()
        ...
        TriggerClientEvent('midp-phone:client:UpdateMails', src, mails)
    end)
end)
...
RegisterNetEvent('midp-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    ...
    TriggerClientEvent('midp-phone:client:NewMailNotify', src, mailData)
...
RegisterNetEvent('midp-phone:server:sendNewMailToOffline', function(license, mailData)
...
RegisterNetEvent('midp-phone:server:sendNewEventMail', function(license, mailData)
    local Player = ESX.GetPlayerFromIdentifier(license)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`identifier`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', {license, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0})
    else
    ...
    SetTimeout(200, function()
        ...
        TriggerClientEvent('midp-phone:client:UpdateMails', Player.PlayerData.source, mails)
    end)
end)

RegisterNetEvent('midp-phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
RegisterNetEvent('midp-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = ESX.GetPlayerFromId(src)
    ...
    if Target ~= nil then
        TriggerClientEvent('midp-phone:client:GetCalled', Target.source, character.phone, CallId, AnonymousCall)
    end
end)

lib.callback.register("midp-phone:server:GetWebhook",function(_)
	if WebHook ~= "" then
		return WebHook
	else
...
lib.callback.register('midp-phone:server:GetBankData', function(source)
    local src = source
...
lib.callback.register('midp-phone:server:CanPayInvoice', function(source, amount)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
lib.callback.register('midp-phone:server:GetInvoices', function(source)
    local Player = ESX.GetPlayerFromId(source)
...
lib.callback.register('midp-phone:server:GetContactPictures', function(_, Chats)
...
lib.callback.register('midp-phone:server:GetContactPicture', function(_, Chat)
...
lib.callback.register('midp-phone:server:getBackground', function(source)
    local Player = ESX.GetPlayerFromId(source)
...
lib.callback.register('midp-phone:server:getAvatar', function(source)
    local Player = ESX.GetPlayerFromId(source)
...
lib.callback.register('midp-phone:server:GetPicture', function(_, number)
    local Picture = nil
    local query = '%' .. number .. '%'
    local result = MySQL.query.await('SELECT profilepicture FROM users WHERE phone LIKE ?', {query})
    if result[1] ~= nil then
...
RegisterNetEvent('midp-phone:server:savedocuments', function(data)
    local src = source
    local Ply = ESX.GetPlayerFromId(src)
    if data.Type == "New" then
        ...
        TriggerClientEvent('midp-tasknotify:client:SendAlert', src, {type='success', text ='Note Tersimpan'})
    ...
    elseif data.Type == "Update" then
        ...
        if Note[1] ~= nil then
            ...
            MySQL.insert('INSERT INTO phone_note (identifier, title,  text) VALUES (?, ?, ?)',{Ply.identifier, data.Title, data.Text})
            TriggerClientEvent('midp-tasknotify:client:SendAlert', src, {type='success', text ='Note Updated'})
        end

    elseif data.Type == "Delete" then
        local ID = tonumber(data.ID)
        MySQL.update('DELETE FROM phone_note WHERE id = ?', {ID})
        TriggerClientEvent('midp-tasknotify:client:SendAlert', src, {type='success', text ='Note Dihapus'})
    end

    Wait(100)

    TriggerClientEvent('midp-phone:getdocuments', src)
end)

lib.callback.register('midp-phone:server:GetNote_for_Documents_app', function(source)
    local src = source
    local Ply = ESX.GetPlayerFromId(src)
...
RegisterNetEvent('midp-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local Identifier = ESX.GetPlayerFromId(src).identifier
    MIPhone.SetPhoneAlerts(Identifier, app, alerts)
end)

RegisterNetEvent('midp-phone:server:TransferMoney', function(iban, amount)
    local src = source
            ...
            if result[1] ~= nil then
                ...
                if recieverSteam ~= nil then
                    ...
                    if PhoneItem ~= nil then
                        TriggerClientEvent('midp-phone:client:TransferMoney', recieverSteam.source, amount, recieverSteam.getAccount('bank').money)

                        ExecuteSql(false, "SELECT * FROM `users` WHERE `identifier`='"..ESX.GetPlayerFromId(src).identifier.."'", function(result)
                            TriggerClientEvent('midp-tasknotify:client:SendAlert', sender.source, {type='success', text ='Transfer Berhasil'})
                            TriggerClientEvent('midp-tasknotify:client:SendAlert', recieverSteam.source, {type='success', text ='Menerima Transfer '..amount..' Dari: '..result[1].iban})
                        end)
                    end
                else
                ...
                TriggerClientEvent('midp-tasknotify:client:SendAlert', src, {type='error', text ='Rekening Tidak Terdaftar'})
            end
        end)
    else
        TriggerClientEvent('midp-phone:client:NotiftfRWT', src)
    end
end)

RegisterNetEvent('midp-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, _)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    MySQL.update(
...
RegisterNetEvent('midp-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
RegisterNetEvent('midp-phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
RegisterNetEvent('midp-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, New)
    local src = source
    ...
    if Player[1] ~= nil then
        ...
        if TargetData ~= nil then
            ...
            if Chat[1] ~= nil then
                ...
                TriggerClientEvent('midp-phone:client:UpdateMessages', TargetData.source, ChatMessages, SenderCharacter.phone, false)
            else
                -- Insert for target
                ...
                -- Send notification & Update messages for target
                TriggerClientEvent('midp-phone:client:UpdateMessages', TargetData.source, ChatMessages, SenderCharacter.phone, true)
            end
        else
            local Chat = MySQL.query.await('SELECT * FROM phone_messages WHERE identifier = ? AND number = ?', {SenderData.identifier, ChatNumber})
            if Chat[1] ~= nil then
...
RegisterNetEvent('midp-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = ESX.GetPlayerFromId(src)
    local character = GetCharacter(src)
    ...
    TriggerClientEvent('midp-phone:client:AddRecentCall', src, data, label, type)

    local Trgt = GetPlayerFromPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('midp-phone:client:AddRecentCall', Trgt.source, {
...
RegisterNetEvent('midp-phone:server:CancelCall', function(ContactData)
    local Ply = GetPlayerFromPhone(ContactData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('midp-phone:client:CancelCall', Ply.source)
    end
end)

RegisterNetEvent('midp-phone:server:AnswerCall', function(CallData)
    local Ply = GetPlayerFromPhone(CallData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('midp-phone:client:AnswerCall', Ply.source)
    end
end)

RegisterNetEvent('midp-phone:server:SaveMetaData', function(column, data)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
lib.callback.register('midp-phone:server:FetchResult', function(source, search)
    local src = source
...
lib.callback.register('midp-phone:server:GetVehicleSearchResults', function(source, search)
    local src = source
    local search = escape_sqli(search)
    local searchData = {}
...
lib.callback.register('midp-phone:server:ScanPlate', function(source, plate)
    local src = source
    local vehicleData = {}
    ...
    if plate ~= nil then 
        ...
        TriggerClientEvent('midp-tasknotify:client:SendAlert', src, {type='error', text ='No Vehicle Nearby'})
        return nil
    end
end)
...
lib.callback.register('midp-phone:server:GetGarageVehicles', function(source)
    local Player = ESX.GetPlayerFromId(source)
    local Vehicles = {}
...
lib.callback.register('midp-phone:server:GetCharacterData', function(source, id)
    local src = source or id

    return GetCharacter(src)
end)

RegisterNetEvent('midp-phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    local character = GetCharacter(src)
    ...
    TriggerClientEvent('midp-phone:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterNetEvent('midp-phone:server:addImageToGallery', function(image)
    local src = source
   local Player = ESX.GetPlayerFromId(src)
    MySQL.insert('INSERT INTO phone_gallery (`identifier`, `image`) VALUES (?, ?)',{Player.identifier,image})
end)

RegisterNetEvent('midp-phone:server:getImageFromGallery', function()
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    local images = MySQL.query.await('SELECT * FROM phone_gallery WHERE identifier = ? ORDER BY `date` DESC',{Player.identifier})
    TriggerClientEvent('midp-phone:refreshImages', src, images)
end)

RegisterNetEvent('midp-phone:server:RemoveImageFromGallery', function(data)
    local src = source
   local Player = ESX.GetPlayerFromId(src)
...
RegisterServerEvent('midp-phone:addsms', function(coords, pesan, jobs, time)
    ...
    TriggerClientEvent("midp-phone:client:addPoliceAlert", -1, { title = source .. " | #" .. getNumberPhone(steam) .. " (" .. date.hour .. ':' .. date.min .. ")", phone = getNumberPhone(steam),  coords = {x = coords.x, y = coords.y, z = coords.z}, description = pesan }, jobs)
end)

lib.callback.register('midp-phone:server:GetCurrentLawyers', function(source)
    local Lawyers = {}
