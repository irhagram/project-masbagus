...
function EmoteMenu.RequestSynchronizedEmote(senderData)
    ...
    TriggerServerEvent('alan-emotes:requestSynchronizedEmote', GetPlayerServerId(targetId), senderData, emote)
end
...
function EmoteMenu.Play(data, variation)
    ...
    if data.Options then
        ...
        if data.Options.Ptfx then
            ...
            TriggerServerEvent('alan-emotes:syncPtfx', data.Options.Ptfx.Asset, data.Options.Ptfx.Name, data.Options.Ptfx.Placement, data.Options.Ptfx.Color)
        else
        ...
        if propCount > 0 then
            ...
            for i = 1, propCount do
                ...
                if (i == 1) and (data.Options.Ptfx and data.Options.Ptfx.AttachToProp) then
                    TriggerServerEvent('alan-emotes:syncProp', ObjToNet(object))
                end
            end
        end
    end
...
function EmoteMenu.CancelAnimation()
    if EmoteMenu.IsPlayingAnimation then
        ...
        if EmoteMenu.OtherPlayer then
            TriggerServerEvent('alan-emotes:cancelSynchronizedEmote', EmoteMenu.OtherPlayer)
...
exports["alan-core"]:registerKeyMapping('Emote Menu', Config.MenuOpenCommands[1], '')
...
-- Events
RegisterNetEvent('alan-emotes:cancelAnimation', function()
    EmoteMenu.CancelAnimation()
end)

RegisterNetEvent('alan-emotes:closeMenu', function()
    EmoteMenu.CloseMenu()
end)

RegisterNetEvent('alan-emotes:play', function(emoteData, emoteVariant)
    EmoteMenu.Play(emoteData, emoteVariant)
end)

RegisterNetEvent('alan-emotes:playByCommand', function(emoteCommand, emoteVariant)
    EmoteMenu.PlayByCommand(emoteCommand, emoteVariant)
end)

RegisterNetEvent('alan-emotes:playRegisteredEmote', function(emoteName)
    EmoteMenu.PlayRegisteredEmote(emoteName)
end)

RegisterNetEvent('alan-emotes:registerEmote', function(emoteData)
    EmoteMenu.RegisterEmote(emoteData)
end)

RegisterNetEvent('alan-emotes:resetExpression', function()
    EmoteMenu.ResetExpression()
end)

RegisterNetEvent('alan-emotes:setExpression', function(expression)
    EmoteMenu.SetExpression(expression)
end)

RegisterNetEvent('alan-emotes:resetWalk', function()
    EmoteMenu.ResetWalk()
end)

RegisterNetEvent('alan-emotes:setWalk', function(walk)
    EmoteMenu.SetWalk(walk)
end)

RegisterNetEvent('alan-emotes:toggleLimitation', function(_state)
    EmoteMenu.ToggleLimitation(_state)
end)

RegisterNetEvent('alan-emotes:toggleMenu', function()
    EmoteMenu.ToggleMenu()
end)

local IsControlJustPressed = IsControlJustPressed

RegisterNetEvent('alan-emotes:synchronizedEmoteRequest', function(sender, senderData, targetData)
    ...
    while true do
        ...
        if IsControlJustPressed(0, 246) then
            lib.hideTextUI()
            TriggerServerEvent('alan-emotes:synchronizedEmoteResponse', sender, senderData, targetData)
            break
...
RegisterNetEvent('alan-emotes:targetStartSynchronizedEmote', function(sender, senderData, targetData)
...
RegisterNetEvent('alan-emotes:senderStartSynchronizedEmote', function(target, senderData)
...
RegisterNetEvent('alan-emotes:cancelSynchronizedEmote', function()
