...
AddEventHandler("alan-fleecarob:outcome", function(oc, arg)
    ...
    elseif not oc then
        exports['alan-tasknotify']:Alert("SYSTEM", arg, 5000, 'error')
...
AddEventHandler("alan-fleecarob:reset", function(name, data)
    ...
    exports['alan-tasknotify']:Alert("SYSTEM", "PINTU TERTUTUP!", 5000, 'error')
...
AddEventHandler("alan-fleecarob:startheist", function(data, name)
    ...
    Citizen.Wait(1000)
    Wait(2000)
    exports['alan-tasknotify']:Alert("SYSTEM", "HACKING BERHASIL!", 5000, 'success')
    ...
    for k, v in pairs(Config_Fleeca.Banks) do
        if not v.onaction then
            ...
            if dst <= 1 and not Check[k] then
                Wait(750)
                exports['alan-context']:OpenHackingGame(function(outcome)
                    ...
                    elseif outcome == false then
                        exports['alan-tasknotify']:Alert("SYSTEM", "HACKING GAGAL!", 5000, 'error')
                    end
                end)
            end
        end
...
AddEventHandler("alan-fleecarob:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" then
        exports['alan-tasknotify']:Alert("SYSTEM", "ALARM BERBUNYI, MAYDAY!", 5000, 'error')
