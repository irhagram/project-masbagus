...
AddEventHandler('jn-drugs:pickedUpCoke', function()
	...
	if xPlayer.canCarryItem('coke') then
		...
		TriggerClientEvent('alan-tasknotify:client:SendAlert', _source, { type = 'error', text = 'Inventory penuh'})
	end
end)
...
AddEventHandler('jn-drugs:processCoke', function()
	...
	if CopsConnected < Config.PolisiDibutuhkan then
		TriggerClientEvent('alan-tasknotify:client:SendAlert', _source, { type = 'error', text = 'Tidak Cukup Polisi!'})
		return
	end
			...
			if xCoke.count > 3 then
				if xPlayer.canSwapItem('coke', 6, 'coke_pooch', 1) then
					...
					TriggerClientEvent('alan-tasknotify:client:SendAlert', _source, { type = 'success', text = 'Menerima 1X Coke'})
				else
					TriggerClientEvent('alan-tasknotify:client:SendAlert', _source, { type = 'error', text = 'gagal!'})
				end
			else
				TriggerClientEvent('alan-tasknotify:client:SendAlert', _source, { type = 'error', text = 'Kamu Harus Memiliki 6 coca Leaf!'})
			end
		...
		playersProcessingCannabis[_source] = ESX.SetTimeout(Config.Delays.CannabisProcessing, function()
			...
			if xCannabis.count > 3 then
				if xPlayer.canSwapItem('weed', 6, 'weed_pooch', 1) then
					...
					TriggerClientEvent('alan-tasknotify:client:SendAlert', _source, { type = 'success', text = 'Menerima 1X Olahan KEcubung'})
				else
					TriggerClientEvent('alan-tasknotify:client:SendAlert', _source, { type = 'error', text = 'gagal!'})
				end
			else
				TriggerClientEvent('alan-tasknotify:client:SendAlert', _source, { type = 'error', text = 'Kamu Harus Memiliki 6 Buah Kecubung!'})
			end
...
local function SellWeed(source)
	if CopsConnected < Config.PolisiDibutuhkan then
		TriggerClientEvent('alan-tasknotify:Alert', source, "SYSTEM", "Tidak cukup polisi!", 5000, 'error')
		return
	end
	...
	SetTimeout(Config.WaktuJualan, function()
		...
		if PlayersSellingWeed[source] == true then
			...
			if poochQuantity == 0 then
				TriggerClientEvent('alan-tasknotify:Alert', source, "SYSTEM", "Tidak cukup barang!", 5000, 'error')
			else
...
AddEventHandler('jn-drugs:startSellWeed', function(slwd)
	...
	irham.cek(src, slwd, function()
		...
		TriggerClientEvent('alan-tasknotify:Alert', src, "SYSTEM", "Jual DImulai!", 5000, 'error')
		SellWeed(src)
	end, function()
...
local function SellCoke(source)
	...
	SetTimeout(Config.WaktuJualan, function()
		if PlayersSellingCoke[source] == true then
			...
			if poochQuantity == 0 then
				TriggerClientEvent('alan-tasknotify:Alert', _source, "SYSTEM", "Tidak cukup barang!", 5000, 'error')
			else
...
AddEventHandler('jn-drugs:startSellCoke', function(slck)
	...
	irham.cek(src, slck, function()
		...
		TriggerClientEvent('alan-tasknotify:Alert', src, "SYSTEM", "Penjualan dimulai!", 5000, 'error')
		SellCoke(src)
	end, function()
		DropPlayer(src, 'Mau Ngetes ILMU KIDS?')
	end)
