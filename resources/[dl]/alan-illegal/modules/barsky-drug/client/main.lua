...
RegisterNetEvent('alan-illegal:Ngumbah', function()
    ESX.TriggerServerCallback('dl-jobs:cekItem', function(items)
        if items >= 500000 then
            if not sibuk then
            sibuk = true
            TriggerServerEvent('alan-core:delItem', 'black_money', 500000)
            exports.ox_inventory:Progress({
                ...
                if not cancel then
                    TriggerServerEvent('alan-core:NambahItems', 'money', 500000)
                    sibuk = false
                else
                    TriggerServerEvent('alan-core:NambahItems', 'black_money', 500000)
                    sibuk = false
            ...
            exports['alan-tasknotify']:DoHudText('error', 'Minimal cuci uang merah $DL 500.000')
        end
    end, 'black_money')
end)
        ...
        if not sibuk then
            ...
            exports.ox_inventory:Progress({
                ...
                if not cancel then
                    ...
                    exports['alan-tasknotify']:DoHudText('error', 'Perbaikan Senjata Berhasil!')
                    sibuk = false
                else
                    sibuk = false
                    TriggerServerEvent('alan-core:NambahItems', 'repairkit_senjata', 1)
                    exports['alan-tasknotify']:DoHudText('error', 'Perbaikan Senjata Dibatalkan!')
                end
            end)
        end
    else
        exports['alan-tasknotify']:DoHudText('error', 'Tidak Memiliki Repairkit Senjata!')
    end
end)
