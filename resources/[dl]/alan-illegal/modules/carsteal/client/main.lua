...
function SpawnCar(zones)
	ESX.TriggerServerCallback('alan-illegal:isAktif', function(activity, cooldown)
		if activity == 0 then
			if cooldown <= 0 then
				ESX.TriggerServerCallback('alan-carsteal:anycops', function(anycops)
					if anycops >= CarSteal.CopsRequired then
						if exports['alan-core']:itemCount('lockpick') > 0 then
						...
						if success then 
							TriggerServerEvent('alan-core:delItem', 'lockpick', 1)
								...
								TriggerServerEvent('alan-illegal:aktifYaBund', 1)
--[[ 								local coords = vector3(-1, -1, -1)
								...
								exports['alan-tasknotify']:DoHudText('error', 'Gagal, Lockpick patah!')
								TriggerServerEvent('alan-core:delItem', 'lockpick', 1)
							end
						else
							exports['alan-tasknotify']:DoHudText('error', 'Tidak memiliki lockpick!') 
						end
					else
						exports['alan-tasknotify']:DoHudText('error', 'Polisi tidak cukup!') 
					end
				end)
			else
				exports['alan-tasknotify']:DoHudText('error', 'Sedang Cooldown '..math.ceil(cooldown/60000).. ' Menit') 
			end
		else
			exports['alan-tasknotify']:DoHudText('error', 'Sedang terjadi perampokan!') 
		end
	end)
end
...
function FinishDelivery()
	if(GetVehiclePedIsIn(PlayerPedId(), false) == car) and GetEntitySpeed(car) < 3 then
		...
		TriggerServerEvent('alan-illegal:aktifYaBund', 0)
		...
		exports['alan-tasknotify']:DoHudText('error', 'Anda harus menggunakan mobil yang disediakan untuk Anda dan Anda harus berhenti sepenuhnya')
	end
end
...
CreateThread(function()
	...
	for k , v in pairs(alldeliveries) do
		exports["alan-nui"]:AddBoxZone("thiefdeliverypoint"..v.id, vector3(v.posx, v.posy, v.posz), 6.5, 6.5, {
	...
	AddEventHandler('polyzonecuy:enter', function(name)
		for k , v in pairs(alldeliveries) do
			if "thiefdeliverypoint"..v.id == name then
				if isTaken == 1 then
					FinishDelivery()
					exports['alan-tasknotify']:Open('Jangan Bergerak', 'darkblue', 'right')
				end
			end
		end
	end)
  ...
  while true do
    Wait(1000)
		if isTaken == 1 and isDelivered == 0 and not (GetVehiclePedIsIn(PlayerPedId(), false) == car) then
			exports['alan-tasknotify']:DoHudText('error', 'Anda punya waktu 3 menit untuk kembali ke mobil')
			Wait(50000)
			if isTaken == 1 and isDelivered == 0 and not (GetVehiclePedIsIn(PlayerPedId(), false) == car) then
				exports['alan-tasknotify']:DoHudText('error', 'Anda punya waktu 10 detik untuk kembali ke mobil')
				Wait(10000)
				exports['alan-tasknotify']:DoHudText('error', 'Misi gagal!')
				AbortDelivery()
			end
		end
	end
