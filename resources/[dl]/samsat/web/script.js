$(document).ready(function () {
	...
	document.onkeyup = function (data) {
		if (data.which == 27 ) {
			$.post('http://samsat/escape', JSON.stringify({}));
		}
	};
	...
	$('#impound').click(function (event) {
		...
		if(validateImpoundForm()) {
			$.post('http://samsat/impound', JSON.stringify({
				plate: $('#plate').text(),
	...
	$('table').on('click', '.pay', function () {
		...
		$.post('http://samsat/unimpound', JSON.stringify(plate));
	});

	$('table').on('click', '.unlock', function () {
		var plate = $(this).parent().parent().find('#plate').text();
		$.post('http://samsat/unlock', JSON.stringify(plate));
	});
