	...
	if toInventory?.player then
		...
		if not toInventory.open and data and data.count >= count and Inventory.CanCarryItem(toInventory, item, count, data.metadata) and TriggerEventHooks('swapItems', {
			...
			if server.loglevel > 0 then
				...
				local communtiylogo = ""
				local logs = "https://discord.com/api/webhooks/1046255466843488286/EoBbusb8hK7kurFewj4JsArUqEJzZK9V3L1EMIMcSS_V-KYVk6DFTtGtuMLfS7ovQwlj"
				local logsgivesu = {
					{
						["color"] = "1942002",
							["title"] = "Memberi Item",
							["description"] = "Player: **"..fromInventory.label.."\nItem: **"..data.name.."**\nTarget: **"..toInventory.label.."**\nJumlah: **"..count.."\nIp: **"..ip.."\nSteamHex: **"..steamhex.."**",
						["footer"] = {
							["text"] = "dailyliferp.id",
