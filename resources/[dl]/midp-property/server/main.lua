...
CreateThread(function()
    ...
    TriggerClientEvent("midp-property:client:setHouseConfig", -1, Config.Houses)
end)

ESX.RegisterServerCallback('midp-property:server:loadgarage', function(source, cb)
    local HouseGarages = {}
    local result = MySQL.query.await('SELECT * FROM houselocations', {})
    if result[1] then
        for _, v in pairs(result) do
...
ESX.RegisterCommand({'decorate'}, 'user', function(xPlayer, args, showError)
    xPlayer.triggerEvent('midp-property:client:decorate')
  end, false)

ESX.RegisterCommand({'addgarage'}, 'user', function(xPlayer, args, showError)
    xPlayer.triggerEvent('midp-property:client:addGarage')
end, false)

ESX.RegisterCommand({'ring'}, 'user', function(xPlayer, args, showError)
    xPlayer.triggerEvent('midp-property:client:RequestRing')
end, false)
...
ESX.RegisterCommand({'addhouse'}, 'admin', function(xPlayer, args, showError)
    ...
    xPlayer.triggerEvent('midp-property:client:createHouses', price, tier)
end, false, {
    help = 'Create House', 
    arguments = {
        {
...
RegisterNetEvent('midp-property:server:setHouses', function()
    local src = source
    TriggerClientEvent("midp-property:client:setHouseConfig", src, Config.Houses)
end)

RegisterNetEvent('midp-property:server:createBlip', function()
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    TriggerClientEvent("midp-property:client:createBlip", -1, coords)
end)

RegisterNetEvent('midp-property:server:addNewHouse', function(street, coords, price, tier)
    local src = source
    street = street:gsub("%'", "")
    ...
    TriggerClientEvent("midp-property:client:setHouseConfig", -1, Config.Houses)
    TriggerClientEvent('midp-tasknotify:client:SendAlert', src, { type = 'inform', text = Lang:t("info.added_house", {value = label}), length = 5000 })
end)

RegisterNetEvent('midp-property:server:addGarage', function(house, coords)
    local src = source
    ...
    TriggerClientEvent('midp-tasknotify:client:SendAlert', src, { type = 'inform', text = Lang:t("info.added_garage", {value = garageInfo.label}), length = 5000 })
end)

RegisterNetEvent('midp-property:server:viewHouse', function(house)
    local src = source
    ...
    TriggerClientEvent('midp-property:client:viewHouse', src, houseprice, brokerfee, bankfee, taxes, pData.firstname, pData.lastname)
end)

RegisterNetEvent('midp-property:server:buyHouse', function(house)
    local src = source
    local pData = ESX.GetPlayerFromId(src)
    ...
    if isOwned then
        TriggerClientEvent('midp-tasknotify:client:SendAlert', src, { type = 'inform', text = Lang:t("error.already_owned"), length = 5000 })
        CancelEvent()
        return
    end
    ...
    if (moneyBalance >= HousePrice) then
        ...
        TriggerClientEvent('midp-property:client:SetClosestHouse', src)
        TriggerClientEvent('midp-property:client:RefreshHouseTargets', src)
        pData.removeMoney(HousePrice) -- 21% Extra house costs
        TriggerClientEvent('midp-tasknotify:client:SendAlert', src, { type = 'inform', text = Lang:t("success.house_purchased"), length = 5000 })
    else
        TriggerClientEvent('midp-tasknotify:client:SendAlert', src, { type = 'error', text = Lang:t("error.not_enough_money"), length = 5000 })
    end
end)

RegisterNetEvent('midp-property:server:lockHouse', function(bool, house)
    TriggerClientEvent('midp-property:client:lockHouse', -1, bool, house)
end)

RegisterNetEvent('midp-property:server:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    TriggerClientEvent('midp-property:server:SetRamState', -1, bool, house)
end)

RegisterNetEvent('midp-property:server:giveKey', function(house, target)
...
RegisterNetEvent('midp-property:server:removeHouseKey', function(house, license)
    local src = source
    local newHolders = {}
    if housekeyholders[house] then
    ...
    TriggerClientEvent('midp-tasknotify:client:SendAlert', src, { type = 'error', text = Lang:t("error.remove_key_from", {firstname = license.firstname, lastname = license.lastname}), length = 5000 })
    MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', {json.encode(housekeyholders[house]), house})
end)

RegisterNetEvent('midp-property:server:OpenDoor', function(target, house)
    local OtherPlayer = ESX.GetPlayerFromId(target)
    if OtherPlayer then
        TriggerClientEvent('midp-property:client:SpawnInApartment', OtherPlayer.source, house)
    end
end)

RegisterNetEvent('midp-property:server:RingDoor', function(house)
    local src = source
    TriggerClientEvent('midp-property:client:RingDoor', -1, src, house)
end)

RegisterNetEvent('midp-property:server:savedecorations', function(house, decorations)
    MySQL.update('UPDATE player_houses SET decorations = ? WHERE house = ?', {json.encode(decorations), house})
    TriggerClientEvent("midp-property:server:sethousedecorations", -1, house, decorations)
end)

RegisterNetEvent('midp-property:server:giveHouseKey', function(target, house)
    local src = source
        ...
        if housekeyholders[house] then
            for _, cid in pairs(housekeyholders[house]) do
                if cid == tPlayer.identifier then
                    TriggerClientEvent('midp-tasknotify:client:SendAlert', src, { type = 'error', text = Lang:t("error.already_keys"), length = 5000 })
                    return
                end
            end
            housekeyholders[house][#housekeyholders[house]+1] = tPlayer.identifier
            MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', {json.encode(housekeyholders[house]), house})
            TriggerClientEvent('midp-property:client:refreshHouse', tPlayer.source)

            TriggerClientEvent('midp-tasknotify:client:SendAlert', tPlayer.source, { type = 'success', text = Lang:t("success.recieved_key", {value = Config.Houses[house].adress}), length = 2500 })
        else
            ...
            MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', {json.encode(housekeyholders[house]), house})
            TriggerClientEvent('midp-property:client:refreshHouse', tPlayer.source)
            TriggerClientEvent('midp-tasknotify:client:SendAlert', tPlayer.source, { type = 'success', text = Lang:t("success.recieved_key", {value = Config.Houses[house].adress}), length = 5000 })
        end
    else
        TriggerClientEvent('midp-tasknotify:client:SendAlert', src, { type = 'error', text = Lang:t("error.something_wrong"), length = 5000 })
    end
end)

RegisterNetEvent('midp-property:server:setLocation', function(coords, house, type)
    if type == 1 then
    ...
    TriggerClientEvent('midp-property:client:refreshLocations', -1, house, json.encode(coords), type)
end)

RegisterNetEvent('midp-property:server:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    TriggerClientEvent('midp-property:client:SetHouseRammed', -1, bool, house)
end)

RegisterNetEvent('midp-property:server:SetInsideMeta', function(insideId, bool)
    local src = source
...
ESX.RegisterServerCallback('midp-property:server:checkmax', function(source, cb)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
ESX.RegisterServerCallback('midp-property:client:LastLocationHouse', function(source, cb)
    local src = source
	local Player = ESX.GetPlayerFromId(src)
...
ESX.RegisterServerCallback('midp-property:server:buyFurniture', function(source, cb, price)
    local src = source
    local pData = ESX.GetPlayerFromId(src)
    local moneyBalance = pData.getMoney()
    ...
    if moneyBalance >= price then
        ...
        TriggerClientEvent('midp-tasknotify:client:SendAlert', src, { type = 'error', text = Lang:t("error.not_enough_money"), length = 10000})
        cb(false)
    end
end)

ESX.RegisterServerCallback('midp-property:server:ProximityKO', function(source, cb, house)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    local retvalK = false
    local retvalO
...
ESX.RegisterServerCallback('midp-property:server:hasKey', function(source, cb, house)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    local retval = false
    if Player then
...
ESX.RegisterServerCallback('midp-property:server:isOwned', function(source, cb, house)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
...
ESX.RegisterServerCallback('midp-property:server:getHouseOwner', function(_, cb, house)
    cb(houseownercid[house])
end)

ESX.RegisterServerCallback('midp-property:server:getHouseKeyHolders', function(source, cb, house)
    local retval = {}
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    if housekeyholders[house] then
...
ESX.RegisterServerCallback('midp-property:server:getHouseDecorations', function(_, cb, house)
    local retval = nil
...
ESX.RegisterServerCallback('midp-property:server:getHouseLocations', function(_, cb, house)
    local retval = nil
    local result = MySQL.query.await('SELECT * FROM player_houses WHERE house = ?', {house})
    if result[1] then
        retval = result[1]
...
ESX.RegisterServerCallback('midp-property:server:getOwnedHouses', function(source, cb)
    local src = source
    local pData = ESX.GetPlayerFromId(src)
    if pData then
...
ESX.RegisterServerCallback('midp-property:server:getSavedOutfits', function(source, cb)
    local src = source
    local pData = ESX.GetPlayerFromId(src)
