...
local function SaveDecorations()
	if ClosestHouse then
		...
		TriggerServerEvent("midp-property:server:savedecorations", ClosestHouse, ObjectList)
	end
end
...
local function CheckMovementInput()
	...
	if IsControlJustReleased(0, 21) then -- Left Shift
		...
		exports['midp-tasknotify']:SendAlert('inform', Lang:t("info.speed").. tostring(speeds[curSpeed]))
	end
...
RegisterNetEvent('midp-property:client:decorate', function()
	Wait(500)
	if IsInside then
		if HasHouseKey then
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    ESX.TriggerServerCallback('midp-property:server:buyFurniture', function(isSuccess)
        if isSuccess then
            SetNuiFocus(false, false)
            cursorEnabled = false
            SaveDecorations()
	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				exports['midp-tasknotify']:SendAlert('error', Lang:t("error.out_range"))
			end
		end
	end
end)
