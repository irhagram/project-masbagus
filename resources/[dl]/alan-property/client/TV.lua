...
RegisterNetEvent('loaf_tv:update', function(players)
...
exports["ox_target"]:AddTargetModel(tvasuk, {
options = {
    {
        event = "dl-tv:input",
        icon = "fas fa-tv",
...
RegisterNetEvent('dl-tv:input', function()
    for k, v in pairs(Config['Objects']) do
        local obj = GetClosestObjectOfType(GetEntityCoords(PlayerPedId()), 5.0, GetHashKey(v['Object']))
        if DoesEntityExist(obj) then
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'tv', {
	        	title = "Masukan Youtube ID",
	        }, function (data, menu)
	        	local youtubeid = data.value
	        	if youtubeid == nil then
	        		exports['alan-tasknotify']:Alert('TV', 'ID Youtube Tidak Boleh Kosong', 5000, 'warnings')
	        	else
	        		TriggerServerEvent('loaf_tv:add', youtubeid, v['Object'], GetEntityCoords(obj), v['Scale'], v['Offset'])
                    exports['alan-tasknotify']:Alert('TV', 'Tekan "E" Untuk Sync Waktu Dengan Warga Sekitar', 6000, 'info')
	        		menu.close()
	        	end
...
RegisterNetEvent('dl-tv:volumeset', function()
    ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'setvolume', {
		title = "Masukan Volume (1-10)",
	}, function (data2, menu)
        local volume = tonumber(data2.value)
		if volume == nil then
			exports['alan-tasknotify']:Alert('TV', 'Input Volume Yang Bener', 5000, 'warnings')
		else
            if volume then
                if volume >= 0 then
                    for k, v in pairs(data) do
...
RegisterNetEvent('dl-tv:matikan', function()
    for k, v in pairs(data) do
        if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
...
RegisterNetEvent('loaf_tv:delete', function(id)
    if data[id] then
        if data[id]['DUI'] then
            DestroyDui(data[id]['DUI']['Long'])
        end
...
RegisterNetEvent('loaf_tv:updatevolume', function(id, volume)
    if data[id] then
