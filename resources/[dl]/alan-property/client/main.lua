...
local function showEntranceHeaderMenu()
    ...
    if not isOwned then
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "alan-property:client:ViewHouse",
                args = {}
            }
        }
    else
        if isOwned and HasHouseKey then
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "alan-property:client:EnterHouse",
                    args = {}
                }
            }
           --[[ headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "alan-property:client:giveHouseKey",
                    args = {}
                }
            }]]
        elseif isOwned and not HasHouseKey then
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "alan-property:client:RequestRing",
                    args = {}
                }
            }
            headerMenu[#headerMenu+1] = {
            ...
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
                headerMenu[#headerMenu+1] = {
                    ...
                    params = {
                        event = "alan-property:client:ResetHouse",
                        args = {}
                    }
                }
            end
...
local function showExitHeaderMenu()
    ...
    headerMenu[#headerMenu+1] = {
        ...
        params = {
            event = "alan-property:client:ExitOwnedHouse",
            args = {}
        }
    }
    if isOwned then
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "alan-property:client:FrontDoorCam",
                args = {}
            }
        }
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "alan-property:client:AnswerDoorbell",
                args = {}
            }
        }
    end
...
local function RegisterStashTarget()
    ...
    stashTargetBox:onPlayerInOut(function (isPointInside)
        if isPointInside and not entering and isOwned then
            exports['alan-tasknotify']:Open(Lang:t("target.open_stash"), 'darkblue', 'left')
        else
...
local function RegisterOutfitsTarget()
    ...
    outfitsTargetBox:onPlayerInOut(function (isPointInside)
        if isPointInside and not entering and isOwned then
            exports['alan-tasknotify']:Open(Lang:t("target.outfits"), 'darkblue', 'left')
        else
            exports['alan-tasknotify']:Close()
        end
...
RegisterNetEvent('alan-property:client:setHouseConfig', function(houseConfig)
...
local function SetClosestHouse()
    ...
    if not IsInside then
        ...
        if ClosestHouse ~= nil and tonumber(dist) < 30 then
            ESX.TriggerServerCallback('alan-property:server:ProximityKO', function(key, owned)
                HasHouseKey = key
                isOwned = owned
            end, ClosestHouse)
        end
...
local function setHouseLocations()
    if ClosestHouse ~= nil then
        ESX.TriggerServerCallback('alan-property:server:getHouseLocations', function(result)
            if result ~= nil then
...
local function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		ESX.TriggerServerCallback('alan-property:server:getHouseDecorations', function(result)
...
local function RemoveHouseKey(citizenData)
    TriggerServerEvent('alan-property:server:removeHouseKey', ClosestHouse, citizenData)
    CloseMenuFull()
end
...
local function getKeyHolders()
    ...
    local p = promise.new()
    ESX.TriggerServerCallback('alan-property:server:getHouseKeyHolders', function(holders)
        p:resolve(holders)
    end,ClosestHouse)
...
function HouseKeysMenu()
    ...
    if holders == nil or next(holders) == nil then
        exports['alan-tasknotify']:SendAlert('error', Lang:t("error.no_key_holders"))
        CloseMenuFull()
    else
        keyholderMenu = {}
        ...
        for k, _ in pairs(holders) do
            keyholderMenu[#keyholderMenu+1] = {
                ...
                params = {
                    event = "alan-property:client:OpenClientOptions",
                    args = {
    ...
    keyholderOptions = {
        {
            ...
            params = {
                event = "alan-property:client:RevokeKey",
                args = {
        ...
        },
        {
            ...
            params = {
                event = "alan-property:client:removeHouseKey",
                args = {}
            }
        },
    }
...
local function getDataForHouseTier(house, coords)
    if Config.Houses[house].tier == 1 then
        return exports['alan-property']:CreateFurniMotelModern(coords)
    elseif Config.Houses[house].tier == 2 then
        return exports['alan-property']:CreateFranklinAunt(coords)
    elseif Config.Houses[house].tier == 3 then
        return exports['alan-property']:CreateGarageMed(coords)
    elseif Config.Houses[house].tier == 4 then
        return exports['alan-property']:CreateLesterShell(coords)
    elseif Config.Houses[house].tier == 5 then
        return exports['alan-property']:CreateMichael(coords)
    elseif Config.Houses[house].tier == 6 then
        return exports['alan-property']:CreateOffice1(coords)
    elseif Config.Houses[house].tier == 7 then
        return exports['alan-property']:CreateRanchShell(coords)
    elseif Config.Houses[house].tier == 8 then
        return exports['alan-property']:CreateTrevorsShell(coords)
    elseif Config.Houses[house].tier == 9 then
        return exports['alan-property']:CreateApartmentShell(coords)
    elseif Config.Houses[house].tier == 10 then
        return exports['alan-property']:CreateTier1House(coords)
    elseif Config.Houses[house].tier == 11 then
        return exports['alan-property']:CreateWarehouse1(coords)
    elseif Config.Houses[house].tier == 12 then
        return exports['alan-property']:CreateFurniMotelStandard(coords)
    elseif Config.Houses[house].tier == 13 then
        return exports['alan-property']:CreateHighend1(coords)
    elseif Config.Houses[house].tier == 14 then
        return exports['alan-property']:CreateHighendV2(coords)
    elseif Config.Houses[house].tier == 15 then
        return exports['alan-property']:CreateGarageHigh(coords)
    elseif Config.Houses[house].tier == 16 then
        return exports['alan-property']:CreateHighend2(coords)
    elseif Config.Houses[house].tier == 17 then
        return exports['alan-property']:CreateHighend3(coords)
    elseif Config.Houses[house].tier == 18 then
        return exports['alan-property']:CreateMedium3(coords)
    else
        exports['alan-tasknotify']:SendAlert('error', Lang:t("error.invalid_tier"))
    end
end
...
local function enterOwnedHouse(house)
    ...
    entering = true
    Wait(500)
    TriggerServerEvent('alan-property:server:SetInsideMeta', house, true)
    TriggerEvent('weathersync:client:DisableSync')
...
local function LeaveHouse(house)
    if not FrontCam then
        ...
        DoScreenFadeOut(250)
        Wait(500)
        exports['alan-property']:DespawnInterior(houseObj, function()
            UnloadDecorations()
            TriggerEvent('weathersync:client:EnableSync')
            Wait(250)
            DoScreenFadeIn(250)
            ...
            TriggerServerEvent('alan-property:server:SetInsideMeta', house, false)
            CurrentHouse = nil
...
RegisterNetEvent('alan-property:server:sethousedecorations', function(house, decorations)
...
RegisterNetEvent('alan-property:client:sellHouse', function()
    if ClosestHouse and HasHouseKey then
        TriggerServerEvent('alan-property:server:viewHouse', ClosestHouse)
    end
end)

RegisterNetEvent('alan-property:client:EnterHouse', function()
    local ped = ESX.PlayerData.ped
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent('alan-property:client:RequestRing', function()
    if ClosestHouse ~= nil then
        TriggerServerEvent('alan-property:server:RingDoor', ClosestHouse)
    end
end)
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
    ...
    Wait(200)
    TriggerServerEvent('alan-property:server:setHouses')
    SetClosestHouse()
    TriggerEvent('alan-property:client:setupHouseBlips')

    if Config.UnownedBlips then 
        TriggerEvent('alan-property:client:setupHouseBlips2') 
    end
...
RegisterNetEvent('alan-property:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('alan-property:client:createHouses', function(price, tier)
    ...
    street = street:gsub("%-", " ")
    TriggerServerEvent('alan-property:server:addNewHouse', street, coords, price, tier)
    if Config.UnownedBlips then TriggerServerEvent('alan-property:server:createBlip') end
end)

RegisterNetEvent('alan-property:client:addGarage', function()
    if ClosestHouse ~= nil then
        local pos = GetEntityCoords(ESX.PlayerData.ped)
        local heading = GetEntityHeading(ESX.PlayerData.ped)
        local coords = {
        ...
        TriggerServerEvent('alan-property:server:addGarage', ClosestHouse, coords)
    else
        exports['alan-tasknotify']:SendAlert('error', Lang:t("error.no_house"))
    end
end)

RegisterNetEvent('alan-property:client:toggleDoorlock', function()
    local pos = GetEntityCoords(ESX.PlayerData.ped)
    local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
    if dist <= 1.5 then
        if HasHouseKey then
            if Config.Houses[ClosestHouse].locked then
                TriggerServerEvent('alan-property:server:lockHouse', false, ClosestHouse)
                exports['alan-tasknotify']:SendAlert('success', Lang:t("success.unlocked"))
            else
                TriggerServerEvent('alan-property:server:lockHouse', true, ClosestHouse)
                exports['alan-tasknotify']:SendAlert('error', Lang:t("error.locked"))
            end
        else
            exports['alan-tasknotify']:SendAlert('error', Lang:t("error.no_keys"))
        end
    else
        exports['alan-tasknotify']:SendAlert('error', Lang:t("error.no_door"))
    end
end)

RegisterNetEvent('alan-property:client:RingDoor', function(player, house)
    if ClosestHouse == house and IsInside then
        CurrentDoorBell = player
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "doorbell", 0.1)
        exports['alan-tasknotify']:SendAlert('inform', Lang:t("info.door_ringing"))
    end
end)

RegisterNetEvent('alan-property:client:giveHouseKey', function()
    ...
    if player ~= -1 and distance < 2.5 and ClosestHouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('alan-property:server:giveHouseKey', playerId, ClosestHouse)
        else
            exports['alan-tasknotify']:SendAlert('error', Lang:t("error.no_door"))
        end
    elseif ClosestHouse == nil then
        ...
        exports['alan-tasknotify']:SendAlert('error', Lang:t("error.no_one_near"))
    end
end)

RegisterNetEvent('alan-property:client:removeHouseKey', function()
    if ClosestHouse ~= nil then
        local pedpos = GetEntityCoords(ESX.PlayerData.ped)
        local housedist = #(pedpos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
        if housedist <= 5 then
            ESX.TriggerServerCallback('alan-property:server:getHouseOwner', function(result)
                --ESX.GetPlayerData().license
                if ESX.GetPlayerData().citizenid == result then
                    HouseKeysMenu()
                else
                    exports['alan-tasknotify']:SendAlert('error', Lang:t("error.not_owner"))
                end
            end, ClosestHouse)
        else
            exports['alan-tasknotify']:SendAlert('error', Lang:t("error.no_door"))
...
RegisterNetEvent('alan-property:client:RevokeKey', function(cData)
    RemoveHouseKey(cData.citizenData)
end)

RegisterNetEvent('alan-property:client:refreshHouse', function()
    Wait(100)
    SetClosestHouse()
end)

AddEventHandler('playerSpawned', function()
	ESX.TriggerServerCallback('alan-property:client:LastLocationHouse', function(house)
...
RegisterNetEvent('alan-property:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(ESX.PlayerData.ped)
...
RegisterNetEvent('alan-property:client:enterOwnedHouse', function(house)
    ESX.GetPlayerData(function(PlayerData)
        enterOwnedHouse(house)
	end)
end)

RegisterNetEvent('alan-property:client:LastLocationHouse', function(houseId)
    ESX.GetPlayerData(function(PlayerData)
        enterOwnedHouse(houseId)
	end)
end)

RegisterNetEvent('alan-property:client:setupHouseBlips', function() -- Setup owned on load
    CreateThread(function()
        Wait(2000)
        ESX.TriggerServerCallback('alan-property:server:getOwnedHouses', function(ownedHouses)
            if ownedHouses then
...
RegisterNetEvent('alan-property:client:setupHouseBlips2', function() -- Setup unowned on load
...
RegisterNetEvent('alan-property:client:createBlip', function(coords) -- Create unowned on command
...
RegisterNetEvent('alan-property:client:refreshBlips', function() -- Refresh unowned on buy
...
RegisterNetEvent('alan-property:client:SetClosestHouse', function()
    SetClosestHouse()
end)

RegisterNetEvent('alan-property:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
RegisterCommand('setlokasi', function(source, args)
	TriggerEvent('alan-property:client:setLocation', args[1])
end)

RegisterNetEvent('alan-property:client:setLocation', function(type)
    local ped = ESX.PlayerData.ped
    local pos = GetEntityCoords(ped)
    ...
    if IsInside then
        if HasHouseKey then
            if type == 'setstash' then
                TriggerServerEvent('alan-property:server:setLocation', coords, ClosestHouse, 1)
            elseif type == 'setoutift' then
                TriggerServerEvent('alan-property:server:setLocation', coords, ClosestHouse, 2)
            elseif type == 'setlogout' then
                TriggerServerEvent('alan-property:server:setLocation', coords, ClosestHouse, 3)
            end
        else
            exports['alan-tasknotify']:SendAlert('error', Lang:t("error.not_owner"))
        end
    else
        exports['alan-tasknotify']:SendAlert('error', Lang:t("error.not_in_house"))
    end
end)

RegisterNetEvent('alan-property:client:refreshLocations', function(house, location, type)
...
RegisterNetEvent('alan-property:client:HomeInvasion', function()
    local ped = ESX.PlayerData.ped
    local pos = GetEntityCoords(ped)
                        ...
                        if not Config.Houses[ClosestHouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    TriggerServerEvent('alan-property:server:lockHouse', false, ClosestHouse)
                                    exports['alan-tasknotify']:SendAlert('success', Lang:t("success.home_invasion"))
                                    TriggerServerEvent('alan-property:server:SetHouseRammed', true, ClosestHouse)
                                    TriggerServerEvent('alan-property:server:SetRamState', false, ClosestHouse)
                                ...
                                RamsDone = 0
                                TriggerServerEvent('alan-property:server:SetRamState', false, ClosestHouse)
                                exports['alan-tasknotify']:SendAlert('error', Lang:t("error.failed_invasion"))
                                DoRamAnimation(false)
                            end)
                            TriggerServerEvent('alan-property:server:SetRamState', true, ClosestHouse)
                        else
                            exports['alan-tasknotify']:SendAlert('error', Lang:t("error.inprogress_invasion"))
                        end
                    else
                        exports['alan-tasknotify']:SendAlert('error', Lang:t("error.already_open"))
                    end
                else
                    exports['alan-tasknotify']:SendAlert('error', Lang:t("error.no_house"))
                end
            else
                exports['alan-tasknotify']:SendAlert('error', Lang:t("error.no_police"))
            end
        end)
    else
        exports['alan-tasknotify']:SendAlert('error', Lang:t("error.no_house"))
...
RegisterNetEvent('alan-property:client:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('alan-property:client:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('alan-property:client:ResetHouse', function()
    if ClosestHouse ~= nil then
        if Config.Houses[ClosestHouse].IsRammed == nil then
            Config.Houses[ClosestHouse].IsRammed = false
            TriggerServerEvent('alan-property:server:SetHouseRammed', false, ClosestHouse)
            TriggerServerEvent('alan-property:server:SetRamState', false, ClosestHouse)
        end
        if Config.Houses[ClosestHouse].IsRammed then
            ...
            TriggerServerEvent('alan-property:server:lockHouse', true, ClosestHouse)
            RamsDone = 0
            exports['alan-tasknotify']:SendAlert('success', Lang:t("success.lock_invasion"))
        else
            exports['alan-tasknotify']:SendAlert('error', Lang:t("error.no_invasion"))
        end
    end
end)

RegisterNetEvent('alan-property:client:ExitOwnedHouse', function()
...
RegisterNetEvent('alan-property:client:FrontDoorCam', function()
    local door = vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)
    if CheckDistance(door, 1.5) then
...
RegisterNetEvent('alan-property:client:AnswerDoorbell', function()
    if not CurrentDoorBell or CurrentDoorBell == 0 then
        exports['alan-tasknotify']:SendAlert('error', Lang:t('error.nobody_at_door'))
        return
    end
    local door = vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)
    if CheckDistance(door, 1.5) and CurrentDoorBell ~= 0 then
        TriggerServerEvent("alan-property:server:OpenDoor", CurrentDoorBell, ClosestHouse)
        CurrentDoorBell = 0
    end
end)

RegisterNetEvent('alan-property:client:OpenStash', function()
...
RegisterNetEvent('alan-property:client:ChangeOutfit', function()
...
RegisterNetEvent('alan-property:client:ViewHouse', function()
...
RegisterNetEvent('alan-property:client:KeyholderOptions', function(cData)
    optionMenu(cData.citizenData)
end)

RegisterNetEvent('alan-property:client:RefreshHouseTargets', function ()
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)
...
RegisterNUICallback('HasEnoughMoney', function(cData, cb)
    ESX.TriggerServerCallback('alan-property:server:HasEnoughMoney', function(_)
        cb('ok')
    end, cData.objectData)
end)

RegisterNUICallback('buy', function(_, cb)
    ESX.TriggerServerCallback('alan-property:server:checkmax', function(limit)
        if limit >= 2 then
            exports['alan-tasknotify']:SendAlert('error', Lang:t('error.max_buy'))
        else
            openContract(false)
            disableViewCam()
            Config.Houses[ClosestHouse].owner = true
            if Config.UnownedBlips then TriggerEvent('alan-property:client:refreshBlips') end
            TriggerServerEvent('alan-property:server:buyHouse', ClosestHouse)
            cb("ok")
        end
    end)
end)
        ...
        if IsInside then
            ...
            if isInsideStashTarget then
                ...
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('alan-property:client:OpenStash')
                    exports['alan-tasknotify']:Close()
                end
            end
            ...
            if isInsideOutfitsTarget then
                wait = 0
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('alan-property:client:ChangeOutfit')
                    exports['alan-tasknotify']:Close()
                end
            end
        end
