...
local function SaveDecorations()
	if ClosestHouse then
		...
		TriggerServerEvent("alan-property:server:savedecorations", ClosestHouse, ObjectList)
	end
end
...
local function CheckMovementInput()
	...
	if IsControlJustReleased(0, 21) then -- Left Shift
		...
		exports['alan-tasknotify']:SendAlert('inform', Lang:t("info.speed").. tostring(speeds[curSpeed]))
	end
...
RegisterNetEvent('alan-property:client:decorate', function()
	Wait(500)
	if IsInside then
		if HasHouseKey then
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    ESX.TriggerServerCallback('alan-property:server:buyFurniture', function(isSuccess)
        if isSuccess then
            SetNuiFocus(false, false)
            cursorEnabled = false
            SaveDecorations()
	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				exports['alan-tasknotify']:SendAlert('error', Lang:t("error.out_range"))
			end
		end
	end
end)
