...
AddEventHandler('esx_holdup:tooFar', function()
	...
	exports['alan-tasknotify']:SendAlert('success', 'Perampokan dibatalkan')
end)

RegisterNetEvent('esx_holdup:robberyComplete')
AddEventHandler('esx_holdup:robberyComplete', function(award)
	holdingUp, store = false, ''
	exports['alan-tasknotify']:SendAlert('success', 'Perampokan Berhasil, Mendapat: $DL ' .. ESX.Math.GroupDigits(award), 10000)
end)
...
RegisterNUICallback('win', function(data, cb)
	...
	exports['alan-tasknotify']:SendAlert('success', 'Code: ' .. randomnumb)
...
RegisterNUICallback('lose', function(data, cb)
	...
	onLockpick = false
	exports['alan-tasknotify']:SendAlert('error', 'Gagal')
...
AddEventHandler('dl-storob:resultcode', function(result)
	...
	for index, value in ipairs(robberycode) do
		...
		if tonumber(result) == tonumber(value.code) then
			exports['alan-tasknotify']:SendAlert('success', 'Berhasil')
			table.remove(robberycode, index)

			--TriggerServerEvent('dl-storob:delay')
			TriggerServerEvent('esx_holdup:robberyStarted', startrobbing)
		else
			exports['alan-tasknotify']:SendAlert('error', 'Code tidak valid')
			lose1 = lose1 + 1

			if lose1 == 3 then
				table.remove(robberycode, index)
				exports['alan-tasknotify']:SendAlert('error', 'Harap bobol ulang!')
				lose1 = 0
			end
		end
	end
...
AddEventHandler('dl-storob:startedRobbery', function()
	if not cooldown then
		...
		exports['alan-tasknotify']:SendAlert('error', 'Warung ini telah di rampok')
	end
end)
...
AddEventHandler('dl-storob:startedcode', function()
	ESX.TriggerServerCallback('dl-storob:hack', function(quantity)
		if quantity >= 1 then
			...
			exports['alan-tasknotify']:SendAlert('error', 'Membutuhkan Lockpick')
		end
	end, '')
end)