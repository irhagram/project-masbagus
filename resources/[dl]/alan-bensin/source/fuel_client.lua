...
Citizen.CreateThread(function()
	exports["ox_target"]:AddTargetModel({-164877493, -469694731, -462817101, 1933174915, 1694452750, 1339433404, -2007231801}, {
		options = {
			{
				event = "alan-bensin:isibensin",
				icon = "fas fa-gas-pump",
				label = "ISI BBM"
			},
			{
				event = "alan-bensin:isiminyak",
				icon = "fas fa-gas-pump",
...
RegisterNetEvent('alan-bensin:isibensin')
AddEventHandler('alan-bensin:isibensin', function()
	local ped = PlayerPedId()
	...
	exports["alan-tasknotify"]:DoHudText("error", "Kendaraan terlalu jauh")
	end
end)

RegisterNetEvent('alan-bensin:isiminyak')
AddEventHandler('alan-bensin:isiminyak', function()
	if currentCash >= 20000 then
		TriggerServerEvent('alan-bensin:getbensin', GetPlayerServerId(PlayerId()))
		TriggerServerEvent('fuel:pay', 20000)
	end
end)

AddEventHandler('alan-bensin:isivehjer', function()
	local ped = PlayerPedId()
	...
	TriggerEvent('alan-bensin:isijerigenc')
...
AddEventHandler('alan-bensin:isijerigenc', function()
	local ped = PlayerPedId()
	local vehicle = getveh()
	currentFuel = GetVehicleFuelLevel(vehicle)
			...
			TriggerEvent("mythic_progbar:client:progress", {
				...
				if not status then
					...
					if not Config.UseLegacy then
						...
						TriggerServerEvent('alan-bensin:delbensin', GetPlayerServerId(PlayerId()))
					end
				end
			end)
		end
...
RegisterNetEvent('alan-bensin:compass')
AddEventHandler('alan-bensin:compass', function(status)
    ...
    while true do
        ...
        if IsControlJustReleased(0, 29) and IsPedInAnyVehicle(PlayerPedId()) and GetVehicleClass(GetVehiclePedIsIn(PlayerPedId(), false)) ~= 8 then
             if seatbelt == false then
                ...
                exports['alan-tasknotify']:SendAlert('inform', 'Seat Belt Enabled', 10000)
            else
                PlaySoundFrontend(-1, "Faster_Click", "RESPAWN_ONLINE_SOUNDSET", 1)
                exports['alan-tasknotify']:SendAlert('inform', 'Seat Belt Disabled', 10000)
            end
        ...
        if IsControlJustReleased(0, 137) and isSupir then
             if cruise == false then
                ...
                PlaySoundFrontend(-1, "Faster_Click", "RESPAWN_ONLINE_SOUNDSET", 1)
                exports['alan-tasknotify']:SendAlert('inform', 'Cruise Active at ' .. kmh ..  '/kmh', 10000)
            else
                PlaySoundFrontend(-1, "Faster_Click", "RESPAWN_ONLINE_SOUNDSET", 1)
                exports['alan-tasknotify']:SendAlert('inform', 'Cruise Inactive', 10000)
            end
            cruise = not cruise
