...
function SaveRace()
    ...
    TriggerServerEvent('laprace:server:SaveRace', CreatorData)
...
RegisterNetEvent('laprace:client:UpdateRaceRacerData')
AddEventHandler('laprace:client:UpdateRaceRacerData', function(RaceId, RaceData)
...
RegisterNetEvent('laprace:client:JoinRace')
AddEventHandler('laprace:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('laprace:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        TriggerEvent('notification','You are already in a race.', 2)
    end
end)

RegisterNetEvent('laprace:client:LeaveRace')
AddEventHandler('laprace:client:LeaveRace', function(data)
...
RegisterNetEvent('laprace:client:RaceCountdown')
AddEventHandler('laprace:client:RaceCountdown', function()
    TriggerServerEvent('laprace:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('laprace:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('laprace:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
...
function FinishRace()
    TriggerServerEvent('laprace:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
...
RegisterNetEvent('laprace:client:PlayerFinishs')
AddEventHandler('laprace:client:PlayerFinishs', function(RaceId, Place, acoreChar)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            TriggerEvent('notification',acoreChar.firstname..' is finished on place: '..Place, 2, 3500)
        end
    end
end)
...
RegisterNetEvent('laprace:client:WaitingDistanceCheck')
AddEventHandler('laprace:client:WaitingDistanceCheck', function()
    Wait(1000)
    Citizen.CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            TriggerServerEvent('laprace:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Citizen.Wait(1000)
