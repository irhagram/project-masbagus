	...
	if xPlayer["job"]["name"] == "police" then
		...
		if GetPlayerName(jailPlayer) ~= nil then
			...
			if jailTime ~= nil then
				...
				TriggerClientEvent("esx:showNotification", src, GetPlayerName(jailPlayer) .. " Dipenjara " .. jailTime .. " Bulan!")
				
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent('chat:addMessage', -1, {
							template = '<div class="chat-message-rs"><b>{0}</b>{1} {2} {3}</div>',
							args = { "BERITA PENJARA : ", "" .. Firstname .. " " .. Lastname .. " Dipenjara Selama ", "" .. jailTime .. " Bulan, ", "Karena : " .. table.concat(args, " ") }
						})
					end)
				end
			else
				TriggerClientEvent("esx:showNotification", src, "Waktu Tidak Valid!")
			end
		else
			TriggerClientEvent("esx:showNotification", src, "ID tidak ditemukan!")
		end
	else
...
RegisterCommand("unjail", function(src, args)
	...
	if xPlayer["job"]["name"] == "admin" then
		...
		TriggerClientEvent("esx:showNotification", src, "Tidak memiliki aksess!")
	end
end)
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	if xPlayer.getJob().name ~= 'police' or 'admin' then
	...
	GetRPName(targetSrc, function(Firstname, Lastname)
		TriggerClientEvent('chat:addMessage', -1, {
			template = '<div class="chat-message-rs"><b>{0}</b>{1} {2} {3}</div>',
		   args = { "BERITA PENJARA : ", "" .. Firstname .. " " .. Lastname .. " Dipenjara Selama ", "" .. jailTime .. " Bulan, ", "Karena : " .. jailReason }
	  })
	end)
