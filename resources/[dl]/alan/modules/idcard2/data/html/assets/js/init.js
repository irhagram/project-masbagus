  ...
  window.addEventListener('message', function( event ) {
	if (event.data.action == 'open') {
	  ...
	  if ( type == 'identification' || type == null || type == "drivers_license") {
		...
		$('#name').text(metadata.firstName + ' ' + metadata.lastName);
		...
		$('#signature').text(metadata.firstName + ' ' + metadata.lastName);
		$('#expiry').text(metadata.expireson);
		$('#id-card').css('background', 'url(assets/images/kartu_pasien.png)');
	  }	else if ( type == 'kartu_bpjs' ) {
		$('img').show();
		$('#idnum').text(metadata.citizenid);
		$('#name').css('color', '#282828');
		$('#fname').css('color', '#282828');
		...
		$('#signature').text(metadata.firstName + ' ' + metadata.lastName);
		$('#expiry').text(metadata.expireson);
		$('#id-card').css('background', 'url(assets/images/kartu_bpjs.png)');
	  }
