	...
	elseif data.action == "deposit" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				...
				exports['alan-tasknotify']:DoHudText('error', 'Invalid amount')
			end	
		else
			exports['alan-tasknotify']:DoHudText('error', 'Invalid input')
		end
			...
			if tonumber(data.value) > 0 then
				ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
					...
					elseif isUsed == nil then
						exports['alan-tasknotify']:Alert("BANK", "This IBAN does not exist", 5000, 'error')
						exports['alan-tasknotify']:DoHudText('error', 'This IBAN does not exist')
					end
				end, data.iban:upper())
			else
				exports['alan-tasknotify']:DoHudText('error', 'Invalid amount')
	...
	elseif data.action == "change_iban" then
		if Banking.CustomIBANAllowLetters then
			...
			ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
				...
				elseif isUsed ~= nil then
					--exports['alan-tasknotify']:Alert("BANK", "This IBAN is already in use", 5000, 'error')
					exports['alan-tasknotify']:DoHudText('error', 'This IBAN is already in use')
				end
			end, iban)
		elseif not Banking.CustomIBANAllowLetters then
			if tonumber(data.iban) ~= nil then
				...
				--exports['alan-tasknotify']:Alert("BANK", "You can only use numbers on your IBAN", 5000, 'error')
				exports['alan-tasknotify']:DoHudText('error', 'You can only use numbers on your IBAN')
			end
		end
	elseif data.action == "change_pin" then
		if tonumber(data.pin) ~= nil then
			if string.len(data.pin) == 4 then
				...
				--exports['alan-tasknotify']:Alert("BANK", "Your PIN needs to be 4 digits long", 5000, 'info')
				exports['alan-tasknotify']:DoHudText('error', 'Your PIN needs to be 4 digits long')
			end
		else
			exports['alan-tasknotify']:Alert("BANK", "You can only use numbers", 5000, 'info')
			exports['alan-tasknotify']:DoHudText('error', 'You can only use numbers')
		end
	end
end)
...
AddEventHandler('dl-bank:atm', function ()
	ESX.TriggerServerCallback("okokBanking:GetPIN", function(pin)
		if pin then
			...
			--exports['alan-tasknotify']:Alert("BANK", "Head up to a bank to set a PIN code", 5000, 'info')
			exports['alan-tasknotify']:DoHudText('error', 'Head up to a bank to set a PIN code')
		end
	end)
end)
...
AddEventHandler('alan-tasknotify:Alert', function(text, type)
	exports['alan-tasknotify']:DoHudText(type, text)
end)