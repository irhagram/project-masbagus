...
AddEventHandler("okokBanking:DepositMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('alan-tasknotify:Alert', source, "You have deposited "..amount.."$",  'success')
	else
		TriggerClientEvent('alan-tasknotify:Alert', source, "You don't have that much money on you",  'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
		TriggerClientEvent('alan-tasknotify:Alert', source, "You have withdrawn "..amount.."$",  'success')
	else
		TriggerClientEvent('alan-tasknotify:Alert', source, "You don't have that much money on the bank",  'error')
	end
end)
	...
	if xPlayer.identifier ~= targetIdentifier then
		if amount <= playerMoney then
			...
			if xTarget ~= nil then
				...
				for i=1, #xPlayers, 1 do
				    ...
				    if xForPlayer.identifier == targetIdentifier then
				    	...
				    	TriggerClientEvent('alan-tasknotify:Alert', xPlayers[i], "You have received "..amount.."$ from "..xPlayer.getName(),  'success')
				    end
				end
				TriggerEvent('okokBanking:AddTransferTransaction', amount, xTarget, source)
				TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
				TriggerClientEvent('alan-tasknotify:Alert', source, "You have transferred "..amount.."$ to "..xTarget.getName(),  'success')
			elseif xTarget == nil then
				...
				TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
				TriggerClientEvent('alan-tasknotify:Alert', source, "You have transferred "..amount.."$ to "..targetName,  'success')
		...
		TriggerClientEvent('alan-tasknotify:Alert', source, "You can't send money to yourself",  'error')
	end
end)
...
AddEventHandler("okokBanking:DepositMoneyToSociety", function(amount, society, societyName)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('alan-tasknotify:Alert', source, "You have deposited "..amount.."$ to "..societyName,  'success')
	else
		TriggerClientEvent('alan-tasknotify:Alert', source, "You don't have that much money on you",  'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	if amount <= db.value then
		if db.is_withdrawing == 1 then
			TriggerClientEvent('alan-tasknotify:Alert', _source, "Someone is already withdrawing",  'error')
		else
			...
			TriggerClientEvent('alan-tasknotify:Alert', _source, "You have withdrawn "..amount.."$ from "..societyName,  'success')
		end
	else
		TriggerClientEvent('alan-tasknotify:Alert', _source, "Your society doesn't have that much money on the bank",  'error')
	end
...
AddEventHandler("okokBanking:TransferMoneyToSociety", function(amount, ibanNumber, societyName, society)
	...
	local playerMoney = xPlayer.getAccount('bank').money
		...
		if amount <= playerMoney then
			...
			TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
			TriggerClientEvent('alan-tasknotify:Alert', source, "You have transferred "..amount.."$ to "..societyName,  'success')
		else
			TriggerClientEvent('alan-tasknotify:Alert', source, "You don't have that much money on the bank",  'error')
		end
end)
...
AddEventHandler("okokBanking:TransferMoneyToSocietyFromSociety", function(amount, ibanNumber, societyNameTarget, societyTarget, society, societyName, societyMoney)
	...
	if amount <= societyMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
		TriggerClientEvent('alan-tasknotify:Alert', source, "You have transferred "..amount.."$ to "..societyNameTarget,  'success')
	else
		TriggerClientEvent('alan-tasknotify:Alert', source, "Your society doesn't have that much money on the bank",  'error')
	end
end)
...
AddEventHandler("okokBanking:UpdateIbanDB", function(iban, amount)
	...
	if Banking.IBANChangeCost <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('alan-tasknotify:Alert', source, "IBAN successfully changed to "..iban,  'success')
	else
		TriggerClientEvent('alan-tasknotify:Alert', source, "You need to have "..Banking.IBANChangeCost.."$ in order to change your IBAN",  'error')
	end
end)
...
AddEventHandler("okokBanking:UpdatePINDB", function(pin, amount)
	...
	if Banking.PINChangeCost <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('okokBanking:updateIbanPinChange', source)
		TriggerClientEvent('alan-tasknotify:Alert', source, "PIN successfully changed to "..pin,  'success')
	else
		TriggerClientEvent('alan-tasknotify:Alert', source, "You need to have "..Banking.PINChangeCost.."$ in order to change your PIN", 'error')
	end
end)


RegisterServerEvent('alan-tasknotify:Alert')
AddEventHandler('alan-tasknotify:Alert', function(text, type)
	TriggerClientEvent('alan-tasknotify:client:DoHudText', source, { type = type, text = text } )
end)