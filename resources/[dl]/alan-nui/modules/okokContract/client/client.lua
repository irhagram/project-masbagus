...
AddEventHandler('okokContract:GetVehicleInfo', function(source_playername, date, description, price, source)
	...
	if closestPlayer ~= -1 and playerDistance <= 3.0 then
		...
		if DoesEntityExist(vehicle) and (vehDistance <= 3) then
			...
			exports['alan-tasknotify']:Alert("VEHICLE", "You need to be near a vehicle in order to do that", 10000, 'error')
		end
	else
		ClearPedTasks(PlayerPedId())
		exports['alan-tasknotify']:Alert("VEHICLE", "You need to be near someone in order to do that ", 10000, 'error')
	end
end)
