...
RegisterNetEvent('scenes:send', function(sent)
    scenes = sent
end)
...
RegisterCommand('-scenecreate', function()
    ...
    local scene = exports["alan-context"]:KeyboardInput({
        header = "Add Scene",
        rows = {
            {
                id = 0,
    ...
    TriggerServerEvent('scenes:add', coords, message, color, distance)
end)
...
RegisterCommand('+scenedelete', function()
    ...
    if scene ~= nil then
        TriggerServerEvent('scenes:delete', scene)
    end
end)
...
TriggerServerEvent('scenes:fetch')

exports["alan-core"]:registerKeyMapping('Place Scene ', '+scenecreate', '-scenecreate', ' ')
exports["alan-core"]:registerKeyMapping('Toggle Scenes ', '+scenehide', '-scenehide', ' ')
exports["alan-core"]:registerKeyMapping('Delete Scenes ', '+scenedelete', '-scenedelete', ' ')