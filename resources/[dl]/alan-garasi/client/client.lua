...
AddEventHandler('esx:playerLoaded', function(xPlayer)
    ...
    ESX.TriggerServerCallback('alan-garasi:getOwnedProperties', function(properties)
		userProperties = properties
        DeletePrivateBlips()
        RefreshPrivateBlips()
	end)
    ESX.TriggerServerCallback('alan-garasi:getOwnedProperties1', function(properties)
...
function ListVehiclesMenu(garage)
    ...
    ESX.TriggerServerCallback('alan-garasi:getVehicles', function(vehicles)
        local type = nil
        local options = {}

        if not vehicles then
            lib.registerContext({
                id = 'alan-garasi:MenuGarasi',
                title = namagarasi,
                options = {
                    ['Tidak Ada Kendaraan'] = {}
                }
            ...
            return lib.showContext('alan-garasi:MenuGarasi')
        end
        ...
        lib.showContext('alan-garasi:MenuGarasi')
    end, garage)
end
...
function StockVehicleMenu(garage)
    ...
    if IsPedInAnyVehicle(playerPed, false) then
        ...
        ESX.TriggerServerCallback('alan-garasi:stockv', function(valid)
            if valid then
                TriggerServerEvent('alan-garasi:modifystate', vehicleProps, true, garage)
                ESX.Game.DeleteVehicle(vehicle)

            else
                exports['alan-tasknotify']:SendAlert('error', 'Kendaraan Disimpan!', 10000)
            end
        end, vehicleProps)
    else
        exports['alan-tasknotify']:SendAlert('error', 'ayonima sekali ente!', 10000)
    end
end


RegisterNetEvent('alan-garasi:spawnVeh')
AddEventHandler('alan-garasi:spawnVeh', function(data)
    ...
    ESX.Game.SpawnVehicle(vehicle.model, {
        ...
        exports["alan-bensin"]:SetFuel(callback_vehicle, vehicle.fuelLevel)
    ...
    TriggerServerEvent('alan-garasi:modifystate', vehicle, false)
end)
...
RegisterNetEvent('alan-garasi:spawnVehPound')
AddEventHandler('alan-garasi:spawnVehPound', function(data)
    ESX.TriggerServerCallback('alan-garasi:checkMoney', function(hasEnoughMoney)
        if hasEnoughMoney then
            TriggerServerEvent('alan-garasi:pay', data.price)
            SpawnPoundedVehicle(data.vehicle)
        else
            exports['alan-tasknotify']:SendAlert('error', 'Uang Tidak Cukup!', 5000)
        end
    end, data.price)
end)
...
AddEventHandler('alan-garasi:hasEnteredMarker', function(zone)
    if zone == 'spawn' then
        CurrentAction = 'spawn'
...
AddEventHandler('alan-garasi:hasExitedMarker', function(zone)
    ESX.UI.Menu.CloseAll()
    CurrentAction = nil
end)

function ReturnVehicleMenu()
    ESX.TriggerServerCallback('alan-garasi:loadPrice', function(data)
        vehicles1 = data
    end)

    ESX.TriggerServerCallback('alan-garasi:loadVehicles', function(vehicles)
        local menupound = {}
        local type = nil

        if not vehicles then
            lib.registerContext({
                id = 'alan-garasi:Asuransi',
                title = 'Asuransi',
                options = {
                    ['Tidak Ada Kendaraan'] = {}
                }
            ...
            return lib.showContext('alan-garasi:Asuransi')
        end
        ...
        for key,value in pairs(vehicles) do
            ...
            menupound[key] = {
                title = vehicleName,
                event = 'alan-garasi:spawnVehPound',
                arrow = false,
        ...
        lib.showContext('alan-garasi:Asuransi')
    end)
end
...
function ReturnVehiclePrime()
    ...
    ESX.TriggerServerCallback('alan-garasi:loadVehicles', function(vehicles)
        ...
        if not vehicles then
            ...
            return lib.showContext('alan-garasi:AsuransiPrime')
        end
        ...
        lib.showContext('alan-garasi:AsuransiPrime')
    end)
end
...
CreateThread(function()
    ...
    while true do
        ...
        if isInMarker and not hasAlreadyEnteredMarker then
            ...
            TriggerEvent('alan-garasi:hasEnteredMarker', currentZone)
            exports['alan-tasknotify']:Open('[E] - ' .. CurrentActionMsg, 'darkblue', 'left')
            sleepThread = 5
        end

        if not isInMarker and hasAlreadyEnteredMarker then
            hasAlreadyEnteredMarker = false
            TriggerEvent('alan-garasi:hasExitedMarker', LastZone)
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlPressed(0, 38) and (GetGameTimer() - GUI.Time) > 150 then
                if CurrentAction == 'pound_action_menu' then
                    if exports['alan-core']:Level() > 0 then
                        ReturnVehiclePrime()
                    else
                        ReturnVehicleMenu()
                    end
