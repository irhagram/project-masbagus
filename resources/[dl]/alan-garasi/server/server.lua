RegisterServerEvent('alan-garasi:deletevehicle_sv')
RegisterServerEvent('alan-garasi:modifystate')
RegisterServerEvent('alan-garasi:pay')
RegisterServerEvent('alan-garasi:logging')

-- Vehicle fetch
ESX.RegisterServerCallback('alan-garasi:getVehicles', function(source, cb, garage)
...
ESX.RegisterServerCallback('alan-garasi:stockv', function(source, cb, vehicleProps, garage)
    local isFound = false
...
AddEventHandler('alan-garasi:deletevehicle_sv', function(vehicle)
    TriggerClientEvent('alan-garasi:deletevehicle_cl', -1, vehicle)
end)

-- End vehicle store
-- Change state of vehicle
AddEventHandler('alan-garasi:modifystate', function(vehicle, stored, garage)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('alan-garasi:checkMoney', function(source, cb, price)
    local xPlayer = ESX.GetPlayerFromId(source)
...
-- End funds check
-- Withdraw money
AddEventHandler('alan-garasi:pay', function(price)
    local xPlayer = ESX.GetPlayerFromId(source)

    xPlayer.removeMoney(price)
	TriggerClientEvent('alan-tasknotify:client:SendAlert', source, { type = 'error', text = 'Biaya asuransi $DL ' .. ESX.Math.GroupDigits(price)})
end)
...
RegisterServerEvent('alan-garasi:setVehicle')
AddEventHandler('alan-garasi:setVehicle', function (plate, model)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
--[[
ESX.RegisterServerCallback('alan-garasi:getVeh', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local vehicle = {}
...
ESX.RegisterServerCallback('alan-garasi:loadPrice', function(source, cb)
...
ESX.RegisterServerCallback('alan-garasi:loadVehicles', function(source, cb)
	local s = source
...
ESX.RegisterServerCallback('alan-garasi:getOwnedProperties', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local properties = {}
...
ESX.RegisterServerCallback('alan-garasi:getOwnedProperties1', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local properties = {}
