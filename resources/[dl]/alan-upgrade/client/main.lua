...
AddEventHandler('esx_lscustom:installMod', function()
	...
	TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 7.0, 'impactdrill', 0.5)
end)
...
AddEventHandler('esx_lscustom:cancelInstallMod', function()
	...
	if myCar.modHeadlightColor ~= nil then
		SetVehicleMod(vehicle, 49, myCar.modHeadlightColor, false)
		SetVehicleXenonLightsColour(vehicle, myCar.modHeadlightColor)
	end
end)
...
function UpdateMods(data)
	...
	if data.modType then
		...
		elseif data.modType == 'tyreSmokeColor' then
			...
			ESX.Game.SetVehicleProperties(vehicle, props)
			props = {}
		elseif data.modType == 'modHeadlightColor' then
			props['modHeadlightColor'] = data.modNum
			SetVehicleXenonLightsColour(vehicle, data.modNum)
			ESX.Game.SetVehicleProperties(vehicle, props)
			props = {}
		...
		if data.value == k then
			...
			if v.modType then
				...
				elseif v.modType == 17 then -- TURBO
					...
					table.insert(elements, {label = _label, modType = k, modNum = true})
				elseif v.modType == 49 then
					if IsToggleModOn(vehicle, 22) then
						local headlightColors = {"WHITE","BLUE","ELECTRIC_BLUE","MINT_GREEN","LIME_GREEN","YELLOW","GOLDEN_SHOWER","ORANGE","RED","PONY_PINK","HOT_PINK","PURPLE","BLACKLIGHT"}
						local ownedHeadlight = GetVehicleXenonLightsColour(vehicle)

						for index, value in ipairs(headlightColors) do
							if value ~= "" then
								if (index - 1) == ownedHeadlight then
									table.insert(elements, { label = tostring(headlightColors[index]) .. ' - <span style="color:cornflowerblue;">' .. _U('installed') .. '</span>', 
									modType = k, modNum = index - 1 })
								else
									table.insert(elements, { label = tostring(headlightColors[index]) .. ' - <span style="color:green;">$' .. math.floor(vehiclePrice * v.price / 100) .. ' </span>', 
									modType = k, modNum = index - 1 })
								end
							end
						end
				...
				elseif v.modType == 48 then -- Livery
					...
					local props = {}
					props['modHeadlightColor'] = v.headlightType
					ESX.Game.SetVehicleProperties(vehicle, props)
			...
			if IsPedInAnyVehicle(playerPed, false) then
				...
				for k,v in pairs(Config.Zones) do
					if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < v.Size.x and not lsMenuIsShowed then
						letSleep = false
						isInLSMarker  = true
						exports['alan-tasknotify']:Open('E - Menu', 'darkblue', 'left')
				...
				if IsControlJustReleased(0, 38) and not lsMenuIsShowed and isInLSMarker then
					if (PlayerData.job and PlayerData.job.name == 'mechanic' and PlayerData.job.grade_name == 'ketua' or PlayerData.job.name == 'mechanic' and PlayerData.job.grade_name == 'ahli' or PlayerData.job.name == 'mechanic' and PlayerData.job.grade_name == 'boss' or PlayerData.job.name == 'mechanic' and PlayerData.job.grade_name == 'amatir') or not Config.IsMechanicJobOnly then
						lsMenuIsShowed = true
