...
CreateThread(function()
	for k,v in pairs(pakaianpenjahit) do
		exports["ox_target"]:AddBoxZone("pakaianpenjahit", vector3(v.x, v.y, v.z), 1.2, 1.5, {
		  ...
		  options = {
		  {
			event = "alan-jobs:pakaianpenjahit",
			icon = "fas fa-tshirt",
			label = "Ganti Pakaian",
			job = {
				["tailor"] = 0,
	...
	for k,v in pairs(spawnmobiljahit) do
		exports["ox_target"]:AddBoxZone("mobiljahit1", vector3(v.x, v.y, v.z), 2.0, 2.5, {
		  ...
		  options = {
		  {
			event = "alan-jobs:spawnmobiljahit",
			icon = "fas fa-car",
			label = "Ambil Van",
			job = {
				["tailor"] = 0,
...
RegisterNetEvent('alan-jobs:ambilwool')
AddEventHandler('alan-jobs:ambilwool', function()
	if OnDuty == 0 then
    ...
    ESX.TriggerServerCallback('alan-jobs:canPickUp', function(canPickUp)
        if canPickUp then
            ...
            exports.ox_inventory:Progress({
                ...
                if not cancel then
                    TriggerServerEvent('alan-core:NambahItems', 'wool', 4)
                    sibuk = false
                else
                    sibuk = false
...
RegisterNetEvent('alan-jobs:jahitPenjahit')
AddEventHandler('alan-jobs:jahitPenjahit', function()
	if OnDuty == 0 then
		exports['alan-tasknotify']:DoHudText('error', 'Ganti Baju Terlebih Dahulu')
	else
...
RegisterNetEvent('alan-jobs:jualpakaian')
AddEventHandler('alan-jobs:jualpakaian', function()
	if OnDuty == 0 then
		exports['alan-tasknotify']:DoHudText('error', 'Ganti Baju Terlebih Dahulu')
	else
    if not sibuk then
        ...
        exports.ox_inventory:Progress({
            ...
            if not cancel then
                TriggerServerEvent('alan-jobs:JualBaju', irham.t())
                sibuk = false
            else
                sibuk = false
                exports['alan-tasknotify']:DoHudText('error', 'Proses Dibatalkan!')
...
RegisterNetEvent('alan-jobs:spawnmobiljahit')
AddEventHandler('alan-jobs:spawnmobiljahit', function()
	if OnDuty == 0 then
		exports['alan-tasknotify']:DoHudText('error', 'Ganti Baju Terlebih Dahulu')
	else
...
RegisterNetEvent('alan-jobs:pakaianpenjahit')
AddEventHandler('alan-jobs:pakaianpenjahit', function()
	OnDuty = OnDuty + 1
