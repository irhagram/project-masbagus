...
CreateThread(function()
	...
	for k,v in pairs(spawnmobiltambang) do
		exports["ox_target"]:AddBoxZone("mobiljahit", vector3(v.x, v.y, v.z), 1.2, 2.5, {
		  ...
		  options = {
			{
				event = "alan-jobs:pakaiantambang",
				icon = "fas fa-tshirt",
				label = "Ganti Pakaian",
				job = {
					["miner"] = 0,
		  ...
		  {
			event = "alan-jobs:spawnmobiltambang",
			icon = "fas fa-car",
			label = "Ambil Truk",
			job = {
				["miner"] = 0,
...
RegisterNetEvent('alan-jobs:ambilbatu')
AddEventHandler('alan-jobs:ambilbatu', function()
	if OnDuty == 0 then
		...
		ESX.TriggerServerCallback('alan-jobs:canPickUp', function(canPickUp)
			if canPickUp then
				...
				TriggerServerEvent('alan-core:NambahItems', 'stone', 1)
				sibuk = false
			end
		else
...
RegisterNetEvent('alan-jobs:prosestambang')
AddEventHandler('alan-jobs:prosestambang', function()
	if OnDuty == 0 then
		exports['alan-tasknotify']:DoHudText('error', 'Ganti Baju Terlebih Dahulu')
	else
		ESX.TriggerServerCallback('alan-jobs:cekItem', function(items)
			if items >= 2 then
				...
				TriggerServerEvent('alan-jobs:lebur')
				sibuk = false
			end 
		else
			exports['alan-tasknotify']:DoHudText('error', 'Tidak Memiliki Cukup Batu Bersih!')
			end
		end, 'washed_stone')
	end
end)
...
CreateThread(function()
    exports["alan-nui"]:AddBoxZone("cucibatu", vector3(1610.79, 3873.94, 32.31), 25, 25, {
        name="cucibatu",
		heading = 335,
		debugPoly = false,
		minZ = 29.64,
    ...
    AddEventHandler('polyzonecuy:enter', function(name)
		...
		elseif IsPedInAnyVehicle(ped) or IsPedDeadOrDying(ped) then
			exports['alan-tasknotify']:DoHudText('error', 'Harap Turun Dari Kendaraan')
		else
        if name == 'cucibatu' then
            ...
            CreateThread(function()
                while true do
                        if AreaCuciB then
                            exports['alan-tasknotify']:Open('[E] Cuci Batu', 'darkblue', 'right')
                            if IsControlJustReleased(1, 51) then
								ESX.TriggerServerCallback('alan-jobs:cekItem', function(items)
									if items >= 1 then
										if not sibuk then
										sibuk = true
										TriggerServerEvent('alan-core:delItem', 'stone', 1)
										exports.ox_inventory:Progress({
											duration = 5000,
											label = 'Mencuci Batu...',
											useWhileDead = false,
										...
										TriggerServerEvent('alan-core:NambahItems', 'washed_stone', 1)
										sibuk = false
									end 
								else
									exports['alan-tasknotify']:DoHudText('error', 'Tidak Memiliki Cukup Batu!')
									end
								end, 'stone')
                            end
                        else
...
RegisterNetEvent('alan-jobs:spawnmobiltambang')
AddEventHandler('alan-jobs:spawnmobiltambang', function()
	if OnDuty == 0 then
		exports['alan-tasknotify']:DoHudText('error', 'Ganti Baju Terlebih Dahulu')
	else
...
RegisterNetEvent('alan-jobs:jualcopper')
AddEventHandler('alan-jobs:jualcopper', function()
	if onambil then
	else
		exports.ox_inventory:Progress({
			duration = 5000,
		...
		Wait(500)
		onambil = false
		TriggerServerEvent('alan-jobs:jualTembaga', irham.t())
	end
end)

RegisterNetEvent('alan-jobs:jualiron')
AddEventHandler('alan-jobs:jualiron', function()
	if onambil then
	else
		exports.ox_inventory:Progress({
			duration = 5000,
		...
		Wait(500)
		onambil = false
		TriggerServerEvent('alan-jobs:jualIron', irham.t())
	end
end)

RegisterNetEvent('alan-jobs:jualgold')
AddEventHandler('alan-jobs:jualgold', function()
	if onambil then
	else
		exports.ox_inventory:Progress({
			duration = 5000,
		...
		Wait(500)
		onambil = false
		TriggerServerEvent('alan-jobs:jualGold', irham.t())
	end
end)

RegisterNetEvent('alan-jobs:jualdiamond')
AddEventHandler('alan-jobs:jualdiamond', function()
	if onambil then
	else
		exports.ox_inventory:Progress({
			duration = 5000,
		...
		Wait(5000)
		onambil = false
		TriggerServerEvent('alan-jobs:jualDiamond', irham.t())
	end
end)

RegisterNetEvent('alan-jobs:pakaiantambang')
AddEventHandler('alan-jobs:pakaiantambang', function()
	OnDuty = OnDuty + 1
