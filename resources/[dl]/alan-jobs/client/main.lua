...
RegisterNetEvent('alan-jobs:changeclothes')
AddEventHandler('alan-jobs:changeclothes', function()
...
RegisterNetEvent('alan-jobs:TebangKayu')
AddEventHandler('alan-jobs:TebangKayu', function()
    ESX.TriggerServerCallback('alan-jobs:canPickUp', function(canPickUp)
        if canPickUp then
            ...
            exports.ox_inventory:Progress({
                ...
                if not cancel then
                    TriggerServerEvent('alan-core:NambahItems', 'wood', 5)
                    sibuk = false
                else
                    sibuk = false
                    exports['alan-tasknotify']:DoHudText('error', 'Proses Dibatalkan!')
                end
            end)
        end 
            else
...
RegisterNetEvent('alan-jobs:proseskayu')
AddEventHandler('alan-jobs:proseskayu', function()
    ESX.TriggerServerCallback('alan-jobs:cekItem', function(items)
        if items >= 10 then
            TriggerServerEvent('alan-core:delItem', 'wood', 10)
            if not sibuk then
            sibuk = true
            exports.ox_inventory:Progress({
                duration = 5000,
                ...
                if not cancel then
                    TriggerServerEvent('alan-core:NambahItems', 'cutted_wood', 5)
                    sibuk = false
                else
                    TriggerServerEvent('alan-core:NambahItems', 'wood', 10)
                    sibuk = false
                    exports['alan-tasknotify']:DoHudText('error', 'Proses Dibatalkan!')
                end
            end)
            ...
            exports['alan-tasknotify']:DoHudText('error', 'Tidak Memiliki Cukup Kayu!')
        end
    end, 'wood')
end)

RegisterNetEvent('alan-jobs:KemasKayu')
AddEventHandler('alan-jobs:KemasKayu', function()
    ESX.TriggerServerCallback('alan-jobs:cekItem', function(items)
        if items >= 1 then
            if not sibuk then
            sibuk = true
            TriggerServerEvent('alan-core:delItem', 'cutted_wood', 1)
            exports.ox_inventory:Progress({
                duration = 10000,
                label = 'Mengemas kayu...',
                useWhileDead = false,
                ...
                if not cancel then
                    TriggerServerEvent('alan-core:NambahItems', 'packaged_plank', 4)
                    sibuk = false
                else
                    TriggerServerEvent('alan-core:NambahItems', 'cutted_wood', 1)
                    sibuk = false
                    exports['alan-tasknotify']:DoHudText('error', 'Proses Dibatalkan!')
                end
            end)
            ...
            exports['alan-tasknotify']:DoHudText('error', 'Tidak Memiliki Cukup Kayu Potong!')
        end
    end, 'cutted_wood')
end)

RegisterNetEvent('alan-jobs:JualKayu')
AddEventHandler('alan-jobs:JualKayu', function()
    if not sibuk then
        sibuk = true
        exports.ox_inventory:Progress({
            duration = 5000,
            ...
            if not cancel then
                TriggerServerEvent('alan-jobs:JualinKayu', irham.t())
                sibuk = false
            else 
                sibuk = false
                exports['alan-tasknotify']:DoHudText('error', 'Proses Dibatalkan!')
...
CreateThread(function()
    exports["alan-nui"]:AddBoxZone("fishermanfishspot", vector3(4435.21, 4829.60, 0.34), 250.0, 250.0, {
    ...
    AddEventHandler('polyzonecuy:enter', function(name)
        if name == 'fishermanfishspot' then
            ...
            CreateThread(function()
                while true do
                        if AreaMancing then
                            exports['alan-tasknotify']:Open('[E] Mulai Ambil', 'darkblue', 'right')
                            if IsControlJustReleased(1, 51) then
                                local time = math.random(10,15)
                                local circles = math.random(1,2)
                                local success = exports['alan-lockpick']:StartLockPickCircle(circles, time, success)
                                if success then
                                    if not sibuk then
                                        sibuk = true
                                        exports.ox_inventory:Progress({
                                            ...
                                            if not cancel then
                                                TriggerServerEvent('alan-core:NambahItems', 'fish', math.random(3, 5))
                                                sibuk = false
                                            else 
                                                sibuk = false
                                                exports['alan-tasknotify']:DoHudText('error', 'Proses Dibatalkan!')
                                    ...
                                    exports['alan-tasknotify']:DoHudText('error', 'Ikan Lepas, Silahlkan tangkap Lagi!') 
                                end
                            end
                        else
                            break
...
RegisterNetEvent('alan-jobs:JualIkan')
AddEventHandler('alan-jobs:JualIkan', function()
    if not sibuk then
        sibuk = true
        exports.ox_inventory:Progress({
            duration = 5000,
            ...
            if not cancel then
                TriggerServerEvent('alan-jobs:JualanIkan', irham.t())
                sibuk = false
            else 
                sibuk = false
                exports['alan-tasknotify']:DoHudText('error', 'Proses Dibatalkan!')
...
RegisterNetEvent('alan-jobs:miningoil')
AddEventHandler('alan-jobs:miningoil', function()
    ESX.TriggerServerCallback('alan-jobs:canPickUp', function(canPickUp)
        if canPickUp then
            if not sibuk then
            sibuk = true
            exports.ox_inventory:Progress({
                ...
                if not cancel then
                    ...
                    TriggerServerEvent('alan-core:NambahItems', 'petrol', 5)
                    sibuk = false
                else
                    sibuk = false
                    exports['alan-tasknotify']:DoHudText('error', 'Proses Dibatalkan!')
...
RegisterNetEvent('alan-jobs:ConvOil')
AddEventHandler('alan-jobs:ConvOil', function()
    ESX.TriggerServerCallback('alan-jobs:cekItem', function(items)
        if items >= 5 then
            if not sibuk then
            sibuk = true
            TriggerServerEvent('alan-core:delItem', 'petrol', 5)
            exports.ox_inventory:Progress({
                duration = 7000,
                ...
                if not cancel then
                    TriggerServerEvent('alan-core:NambahItems', 'petrol_raffin', 2)
                    sibuk = false
                else
                    TriggerServerEvent('alan-core:NambahItems', 'petrol', 5)
                    sibuk = false
            ...
            exports['alan-tasknotify']:DoHudText('error', 'Tidak Memiliki Cukup Petrol!')
        end
    end, 'petrol')
end)

RegisterNetEvent('alan-jobs:extogas')
AddEventHandler('alan-jobs:extogas', function()
    ESX.TriggerServerCallback('alan-jobs:cekItem', function(items)
        if items >= 2 then
            if not sibuk then
            sibuk = true
            TriggerServerEvent('alan-core:delItem', 'petrol_raffin', 2)
            exports.ox_inventory:Progress({
                duration = 10000,
                ...
                if not cancel then
                    TriggerServerEvent('alan-core:NambahItems', 'essence', 2)
                    sibuk = false
                else
                    TriggerServerEvent('alan-core:NambahItems', 'petrol_raffin', 2)
                    sibuk = false
            ...
            exports['alan-tasknotify']:DoHudText('error', 'Tidak Memiliki Cukup Petrol Raffin!')
        end
    end, 'petrol_raffin')
end)

RegisterNetEvent('alan-jobs:JualMinyak')
AddEventHandler('alan-jobs:JualMinyak', function()
    if not sibuk then
        sibuk = true
        exports.ox_inventory:Progress({
            duration = 5000,
            ...
            if not cancel then
                TriggerServerEvent('alan-jobs:JualinMinyak', irham.t())
                sibuk = false
            else 
                sibuk = false
                exports['alan-tasknotify']:DoHudText('error', 'Proses Dibatalkan!')
...
RegisterNetEvent('alan-jobs:SpawnKkaYu')
AddEventHandler('alan-jobs:SpawnKkaYu', function()
...
RegisterNetEvent('alan-jobs:vehNelayan')
AddEventHandler('alan-jobs:vehNelayan', function()
...
RegisterNetEvent('alan-jobs:VehFueler')
AddEventHandler('alan-jobs:VehFueler', function()
...
--jobcenter--
RegisterNetEvent('alan-jobs:pilihkerja')
AddEventHandler('alan-jobs:pilihkerja', function()
	local jobs = {}
    ...
    TriggerEvent('alan-context:sendMenu', jobs)      
...
function notify(mensaje)
    ...
    elseif Config.notitype == 'mythic' then
        exports['alan-tasknotify']:SendAlert('inform', mensaje, 10000)
    end
...
RegisterCommand("jangkar", function()
	...
	if not IsPedInAnyVehicle(ped) and boat ~= nil then
		if not jangkar then
			...
			Wait(10000)
			exports['alan-tasknotify']:SendAlert('inform', 'Jangkar dipasang', 10000)
			ClearPedTasks(ped)
		else
			TaskStartScenarioInPlace(ped, 'CODE_HUMAN_MEDIC_TEND_TO_DEAD', 0, true)
			Wait(10000)
			SetBoatAnchor(boat, false)
			exports['alan-tasknotify']:SendAlert('inform', 'Jangkar diambil', 10000)
			ClearPedTasks(ped)
		end
		jangkar = not jangkar
	end
