		...
		if closestPlayer ~= -1 and closestDistance <= 2.0 then
			...
			if action == 'ls_sima' then
                if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					exports['alan-tasknotify']:Alert("SYSTEM", "Tidak ada orang di sekitar!", 1500, 'error')
                end
			...
			exports['alan-tasknotify']:Alert('LISENSI', 'Tidak Ada Orang di Sekitar', 5000, 'error')
		end
		ClearPedTasks(playerped)
		menu.close()
	end, function(data, menu)
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				...
				if chance <= 50 then
					...
					SetVehicleDoorsLockedForAllPlayers(vehicle, false)
					ClearPedTasksImmediately(playerPed)
					--exports['alan-tasknotify']:SendAlert('success', 'Successfully break into the vehicle')
					exports['alan-tasknotify']:Alert("Vehicle Action", "Successfully break into the vehicle", 3000, 'success')
				else
					--exports['alan-tasknotify']:SendAlert('error', 'Failed to break into the vehicle')
					exports['alan-tasknotify']:Alert("Vehicle Action", "Failed to break into the vehicle", 3000, 'error')
					ClearPedTasksImmediately(playerPed)
				end
			end)
		end
