...
AddEventHandler('alan-utility:dorongmobildepan', function(data)
	...
	--exports['alan-tasknotify']:SendAlert('inform', 'Kamu mulai mendorong mobil,tekan W untuk maju dan A/D untuk berbelok dan X untuk berhenti dorong', 10000)
	while true do
		Wait(5)
		...
		if HasEntityCollidedWithAnything(closestVehicle) then
			...
			--exports['alan-tasknotify']:SendAlert('error', 'Kamu tidak bisa mendorong mobil yang terbalik!')
		...
		if IsControlPressed(0, 105) then
			--exports['alan-tasknotify']:SendAlert('error', 'Kamu berhenti mendorong mobil')
			DetachEntity(ped, false, false)
			StopAnimTask(ped, 'missfinale_c2ig_11', 'pushcar_offcliff_m', 2.0)
			FreezeEntityPosition(ped, false)
			break
...
AddEventHandler('esx_ambulancejob:wheelchairspawn', function()
	if wheelchairused then
		...
		if DoesEntityExist(wheelchair) then
			...
			Sexports['alan-tasknotify']:SendAlert('error', 'Terlalu Jauh!')
		end
...
Sit = function(wheelchairObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'missfinale_c2leadinoutfin_c_int', '_leadin_loop2_lester', 3) then
            exports['alan-tasknotify']:SendAlert('error', 'Ada Orang!')
			return
		end
	end
...
RegisterNetEvent('alan-data:tabungo2')
AddEventHandler('alan-data:tabungo2', function()
	...
	if not usingOxygenMask then
		TriggerServerEvent('alan-core:delItem', 'tabungoksigen', 1)
...
RegisterNetEvent('alan-data:Mbledos', function()
            ...
            if closestTire ~= nil then
                if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
                    ...
                    if driverServId == 0 then
                        ...
                        TriggerServerEvent('alan-data:sync', driverServId, closestTire.tireIndex)
                    end
                    Wait((animDuration / 2) * 1000)
                    ClearPedTasks(PlayerPedId())
                    RemoveAnimDict(animDict)
                else
                    exports['alan-tasknotify']:SendAlert('error', 'Ban Sudah Pecah!')
                end
            end
        else
            exports['alan-tasknotify']:SendAlert('error', 'Membutuhkan Senjata Tajam')
        end
    end
end)

RegisterNetEvent('alan-data:sync')
AddEventHandler('alan-data:sync', function(tireIndex)
	exports['alan-tasknotify']:SendAlert('inform', 'Seseorang Memecahkan Ban mu!')
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
