...
CreateThread(function()
	for k,v in pairs(pakaianayam) do
		exports["ox_target"]:AddBoxZone("pakaianayam1", vector3(v.x, v.y, v.z), 1.2, 1.5, {
		  ...
		  {
			event = "midp-jobs:spawnmobilayam",
			icon = "fas fa-car",
			label = "Ambil Truk",
			job = {
                ["slaughterer"] = 0,
...
RegisterNetEvent("midp-jobs:takehewan")
AddEventHandler("midp-jobs:takehewan", function()
	local dead = false
			...
			if AnimalHealth <= 0 and PlyToAnimal < 1.5 then
				if OnDutyAayam > 0 then
					if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_MACHETE')  then
						...
						exports['midp-tasknotify']:DoHudText('error', 'Membutuhkan Parang')
					end
				end
			end
		end
...
function SlaughterAnimal(AnimalId)
	...
	ESX.TriggerServerCallback('midp-jobs:canPickUp', function(canPickUp)
		if canPickUp then
			...
			TriggerServerEvent('midp-core:NambahItems', 'alive_chicken', 4)
			createPedAyam()
			count = count - 1
		else
...
-- EVENT
RegisterNetEvent('midp-jobs:ondutyayam')
AddEventHandler('midp-jobs:ondutyayam', function()
...
RegisterNetEvent('midp-jobs:getmachete')
AddEventHandler('midp-jobs:getmachete', function()
	TriggerServerEvent('midp-jobs:machete')

end)

RegisterNetEvent('midp-jobs:tangkapayam')
AddEventHandler('midp-jobs:tangkapayam', function()
	if OnDutyAayam == 0 then
...
RegisterNetEvent('midp-jobs:prosesAyam')
AddEventHandler('midp-jobs:prosesAyam', function()
	if OnDutyAayam == 0 then
		exports['midp-tasknotify']:DoHudText('error', 'Ganti Baju Terlebih Dahulu')
	else
		ESX.TriggerServerCallback('midp-jobs:cekItem', function(items)
			if items >= 4 then
				if not sibuk then
				sibuk = true
				TriggerServerEvent('midp-core:delItem', 'alive_chicken', 4)
				exports.ox_inventory:Progress({
					duration = 6000,
					label = 'Memotong Ayam...',
					useWhileDead = false,
				...
				TriggerServerEvent('midp-core:NambahItems', 'slaughtered_chicken', 8)
				sibuk = false
			end 
		else
			exports['midp-tasknotify']:DoHudText('error', 'Tidak Memiliki Cukup Ayam!')
			end
		end, 'alive_chicken')
	end
end)

RegisterNetEvent('midp-jobs:kemasayam')
AddEventHandler('midp-jobs:kemasayam', function()
	if OnDutyAayam == 0 then
		exports['midp-tasknotify']:DoHudText('error', 'Ganti Baju Terlebih Dahulu')
	else
		ESX.TriggerServerCallback('midp-jobs:cekItem', function(items)
			if items >= 8 then
				if not sibuk then
				sibuk = true
				TriggerServerEvent('midp-core:delItem', 'slaughtered_chicken', 8)
				exports.ox_inventory:Progress({
					duration = 8000,
					label = 'Mengemas Ayam...',
					useWhileDead = false,
				...
				TriggerServerEvent('midp-core:NambahItems', 'packaged_chicken', 10)
				sibuk = false
			end 
		else
			exports['midp-tasknotify']:DoHudText('error', 'Tidak Memiliki Cukup Ayam Potong!')
			end
		end, 'slaughtered_chicken')
	end
end)

RegisterNetEvent('midp-jobs:jualayam')
AddEventHandler('midp-jobs:jualayam', function()
	if OnDutyAayam == 0 then
		exports['midp-tasknotify']:DoHudText('error', 'Ganti Baju Terlebih Dahulu')
	else
		if onambil then
			...
			Wait(500)
			onambil = false
			TriggerServerEvent('midp-jobs:jualAyam', irham.t())
		end
	end
end)

RegisterNetEvent('midp-jobs:spawnmobilayam')
AddEventHandler('midp-jobs:spawnmobilayam', function()
	if OnDutyAayam == 0 then
		exports['midp-tasknotify']:DoHudText('error', 'Ganti Baju Terlebih Dahulu')
	else
...
RegisterNetEvent('midp-jobs:hapusmobil')
AddEventHandler('midp-jobs:hapusmobil', function()
