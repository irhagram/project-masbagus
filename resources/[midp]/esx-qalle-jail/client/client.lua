...
AddEventHandler("esx-qalle-jail:openJailMenu", function()
	local input = lib.inputDialog('PENJARA', {'ID PLAYER:', 'DURASI:', 'ALASAN:'})

	if not input then return end
	local idplayer = tonumber(input[1])
	local durasi = tonumber(input[2])
	local alasan = input[3]
	TriggerServerEvent("esx-qalle-jail:jailPlayer", idplayer, durasi, alasan)
end)
...
function JailLogin()
	...
	ESX.ShowNotification("Terakhir kali Anda tidur, Anda dipenjara, karena itu sekarang Anda dikembalikan!")
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Waktu penjara " .. jailTime .. " Menit!")

			TriggerServerEvent("esx-qalle-jail:updateJailTime", jailTime)

			if GetDistanceBetweenCoords(GetEntityCoords(playerPed), JailLocation.x, JailLocation.y, JailLocation.z) > 200 then
