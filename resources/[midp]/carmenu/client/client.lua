    ...
    while true do
        ...
        if (IsPedInAnyVehicle(PlayerPedId(), false)) and disableShuffle then
            if (GetIsTaskActive(PlayerPedId(), 165)) then
                SetPedIntoVehicle(PlayerPedId(), GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
                SetVehicleCloseDoorDeferedAction(GetVehiclePedIsIn(PlayerPedId(), 0), 0)
            end
        end

        if not GetPedConfigFlag(PlayerPedId(), 184, 1) and disableShuffle then SetPedConfigFlag(PlayerPedId(), 184, true) end
    end
end)

RegisterNetEvent("veh:EngineToggle")
AddEventHandler("veh:EngineToggle", function()
