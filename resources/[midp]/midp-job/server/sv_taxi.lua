...
if Config.TaxiMaxInService ~= -1 then
	TriggerEvent('esx_service:activateService', 'taxi', Config.TaxiMaxInService)
end
...
AddEventHandler('esx_taxijob:success', function()
	...
	if xPlayer.job.name == 'taxi' then
		if not lastPlayerSuccess[source] or timeNow - lastPlayerSuccess[source] > 5 then
			...
			local total = math.random(Config.TaxiNPCJobEarnings.min, Config.TaxiNPCJobEarnings.max)
