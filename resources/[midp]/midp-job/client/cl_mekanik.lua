					...
					if result then
						...
						Citizen.CreateThread(function()
							...
							if finished ~= 100 then
								ClearPedTasksImmediately(playerPed)
								exports['midp-tasknotify']:DoHudText('error', "Gagal Di Repair")
								isBusy = false
						...
						exports['midp-tasknotify']:DoHudText('error', "Tidak Punya Toolkit")
					end
				end)
			else
				ESX.ShowNotification(_U('no_vehicle_nearby'))
...
AddEventHandler('repairmech', function ()
	...
	if exports['midp-core']:itemCount('toolkit') > 0 then
	if DoesEntityExist(vehicle) then
			if not sibuk then
				sibuk = true
				TriggerServerEvent('midp-core:delItem', 'toolkit', 1)
				exports.ox_inventory:Progress({
					...
					if not cancel then
						...
						exports['midp-tasknotify']:DoHudText('inform', 'Kendaraan Telah Diperbaiki!')
						sibuk = false
					else
						TriggerServerEvent('midp-core:NambahItems', 'toolkit', 1)
						sibuk = false
						exports['midp-tasknotify']:DoHudText('error', 'Perbaikan Dibatalkan!')
					end
				end)
			end
		else
			exports['midp-tasknotify']:DoHudText('error', 'Tidak Ada Kendaraan Disekitar!')
	end
	else
		exports['midp-tasknotify']:DoHudText('error', 'Tidak Memiliki toolkit!')
	end
end)
...
AddEventHandler('cleanmech', function ()
	...
	if exports['midp-core']:itemCount('kanebo') > 0 then
	if DoesEntityExist(vehicle) then
		if not sibuk then
            sibuk = true
			TriggerServerEvent('midp-core:delItem', 'kanebo', 1)
            exports.ox_inventory:Progress({
                duration = 5000,
                ...
                if not cancel then
					SetVehicleDirtLevel(vehicle, 0)
					ClearPedTasksImmediately(playerPed)
					exports['midp-tasknotify']:DoHudText('error', 'Kendaraan Telah Di Cuci!')
                    sibuk = false
                else
                    sibuk = false
					TriggerServerEvent('midp-core:NambahItems', 'kanebo', 1)
                    exports['midp-tasknotify']:DoHudText('error', 'Cuci Dibatalkan!')
                end
            end)
        end
	else
		...
		exports['midp-tasknotify']:DoHudText('error', 'Tidak memiliki kanebo!')
	end
end)
...
RegisterNetEvent('midp-context:DutyMecha', function()
