...
local Webhook = "https://discord.com/api/webhooks/1010273510301651044/nyPs1qPA8S9_VlU_zaVx5kyjvbSnGP0OXAHGMHc_5rASO4NvcexKAtQJa5U0dwBo15pa" -- Put your Discord webhook here to log Play and Saves
...
if Framework == "ESX" then
    ...
    AddEventHandler("wasabi_boombox:DiscordKnows", function(link)
        local xPlayer = ESX.GetPlayerFromId(source)
        SongConfirmed(16448250, "Boombox", "Player: **"..xPlayer.getName().."**\n Identifier: **"..xPlayer.getIdentifier().."**\n Link Lagu: **"..link.."**\n Date: "..os.date("** Time: %H:%M Date: %d.%m.%y **").."", "dailyliferp.id")
    end)
elseif Framework == "QB" then
    RegisterNetEvent("wasabi_boombox:DiscordKnows")
    AddEventHandler("wasabi_boombox:DiscordKnows", function(link)
        local Player = QBCore.Functions.GetPlayer(source)
        local CitizenId = Player.PlayerData.citizenid
        SongConfirmed(16448250, "Boombox", "Player: **"..GetPlayerName(source).."**\n  Player CitizenID: " .. CitizenId .."**\n Song Link: **"..link.."**\n Date: "..os.date("** Time: %H:%M Date: %d.%m.%y **").."", "Made by Andistyler")
    end)
end
