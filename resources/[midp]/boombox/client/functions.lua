...
hasBoomBox = function(radio)
    ...
    CreateThread(function()
        if Config.InstructionNotification then
            lib.notify({
                title = 'Instruksi',
                description = 'Tekan E untuk naruh boombox',
                type = 'success'
            })
        end
        while equipRadio do
        ...
        CreateThread(function()
            while true do
                if DoesEntityExist(obj) and not targetPlaced then
                    exports.ox_target:AddBoxZone("boomboxzone", coords, 1, 1, {
                        name="boomboxzone",
                        heading=heading,
                        debugPoly=false,
                        minZ=coords.z-0.9,
                ...
                elseif not DoesEntityExist(obj) then
                    exports.ox_target:RemoveZone('boomboxzone')
                    targetPlaced = false
                    break
                end
                Wait(1000)
    ...
    savedSongsMenu = function(radio)
        ESX.TriggerServerCallback('wasabi_boombox:getSavedSongs', function(cb)
            ...
            local Options = {
                {
                    title = 'Simpan Lagu',
                    description = 'Simpan lagu untuk diputar nanti',
                    arrow = true,
