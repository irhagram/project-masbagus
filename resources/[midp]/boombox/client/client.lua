...
AddEventHandler('wasabi_boombox:saveSong', function()
    local input = lib.inputDialog('Simpan Lagu', {'Nama', 'Link Youtube'})
    if input[1] and input[2] then
        TriggerServerEvent('wasabi_boombox:save', input[1], input[2])
        lib.notify({
            title = 'Success',
            description = 'Lagu Tersimpan',
            type = 'success'
        })
    else
        lib.notify({
            title = 'Incorrect',
            description = 'Input Yang Bener',
            type = 'error'
        })
    end
end)
...
AddEventHandler('wasabi_boombox:playMenu', function(data)
    ...
    if data.type == 'play' then
        local keyboard = lib.inputDialog('Putar Lagu', {'Youtube URL','Distance (Max 40)', 'Volume (1-100)'})
        if keyboard then
    ...
    elseif data.type == 'volume' then
        local keyboard = lib.inputDialog('Mengubah Volume', {'Volume (1-100)'})    
        if keyboard then
    ...
    elseif data.type == 'distance' then
        local keyboard = lib.inputDialog('Mengubah Distance', {'Distance (Max 40)'})
        if keyboard then
