...
AddEventHandler('rflx_pdblips:setDuty', function(onDuty)
    ...
    if onDuty then
        ...
        if JobBlip.emergencyJobs[playerJob.name] then
            local cfg = JobBlip.emergencyJobs[playerJob.name]

            dutyTable[src] = {
                job = playerJob.name,
                name = JobBlip.useCharacterName and GetCharacterName(src) or GetPlayerName(src),
                prefix = cfg.gradePrefix ~= nil and cfg.gradePrefix[playerJob.grade] ~= nil and cfg.gradePrefix[playerJob.grade] ~= nil and JobBlip.namePrefix[playerJob.grade_name] or '',
...
AddEventHandler('rflx_pdblips:enteredVeh', function(vehCfg)
    ...
    dutyTable[src].vehSprite = vehCfg and vehCfg.sprite or JobBlip.emergencyJobs[playerJob.name].vehBlip['default'].sprite or JobBlip.emergencyJobs[playerJob.name].blip.sprite
    dutyTable[src].vehColor = vehCfg and vehCfg.color or JobBlip.emergencyJobs[playerJob.name].vehBlip['default'].color or JobBlip.emergencyJobs[playerJob.name].blip.color
end)
...
AddEventHandler('rflx_pdblips:toggleSiren', function(isOn)
    ...
    if isOn then
        ...
        dutyTable[src].flashColors = JobBlip.emergencyJobs[playerJob.name].blip.flashColors or {JobBlip.emergencyJobs[playerJob.name].blip.color}
    else
...
if JobBlip.useCharacterName then
