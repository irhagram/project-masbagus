...
AddEventHandler('dl-gudang:tukuGudang', function(gudangName) 
	...
	MySQL.Async.execute('INSERT INTO gudang (identifier, gudang) VALUES (@identifier, @gudang)', {
		...
		TriggerClientEvent('midp-tasknotify:client:SendAlert', rdg, { type = 'error', text = 'Biaya sewa $DL 50.000'})
	end)
end)
...
AddEventHandler('dl-gudang:mandekGudang', function(gudang) 
	...
	MySQL.Async.execute('DELETE FROM gudang WHERE gudang = @gudang AND identifier = @identifier', {
		...
		TriggerClientEvent('midp-tasknotify:client:SendAlert', src, { type = 'error', text = 'Berhenti Menyewa gudang'})
	end)
end)
	...
	MySQL.Async.fetchAll('SELECT * FROM gudang', {}, function(result)
		for i=1, #result, 1 do
			...
			if xPlayer then
				...
				TriggerClientEvent('midp-tasknotify:client:SendAlert', xPlayer.source, { type = 'inform', text = "Membayar Pajak Gudang $DL "..pajakgudang, length = 8000 })
			else
