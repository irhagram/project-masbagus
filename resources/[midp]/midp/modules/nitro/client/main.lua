  ...
  if DoesEntityExist(vehicle) and isBusy == false and IsToggleModOn(vehicle, 18) then
    ...
    exports['mythic_progbar']:Progress({
      ...
      if not cancelled then
        ...
        exports['midp-tasknotify']:DoHudText('success', 'Nitro Terpasang')
      end
      isBusy = false
      ClearPedTasks(PlayerPedId())
    end)
  elseif DoesEntityExist(vehicle) and not IsToggleModOn(vehicle, 18) then
    exports['midp-tasknotify']:DoHudText('error', 'Kendaraan Tidak Memiliki Nitro Storage')
  else
    exports['midp-tasknotify']:DoHudText('error', 'Tidak Ada Kendaraan Di Sekitar')
  end
end)
