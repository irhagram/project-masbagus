...
RegisterNUICallback('simmaker:getDataCivilian' , function(data, cb)
    ESX.TriggerServerCallback('simmaker:cancel' , function(result)
        if #result == 0 then
            exports['midp-tasknotify']:DoHudText('error', 'ID Tidak ditemukkan')
        else
            exports['midp-tasknotify']:DoHudText('success', 'ID ditemukkan')
        end
        if PlayerData.job.name == 'police' then
            result[1].job = 'WARGA'
...
RegisterNUICallback('simmaker:CreateLicense' , function(data, cb)
    ESX.TriggerServerCallback('simmaker:add', function(status)
        if status then
            exports['midp-tasknotify']:DoHudText('success', 'Sukses membuatkan lisensi')
        else
            cb(false)
        end
