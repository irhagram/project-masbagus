...
Citizen.CreateThread(function()
    MySQL.query("SELECT * FROM `lapraces`", {}, function(races)
        if races[1] ~= nil then
...
ESX.RegisterServerCallback('laprace:server:GetRacingLeaderboards', function(source, cb)
    cb(Races)
end)
...
RegisterServerEvent('laprace:server:FinishPlayer')
AddEventHandler('laprace:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    local acoreChar = GetCharacterName(src)
    ...
    if LastRaces[RaceData.RaceId] ~= nil then
        table.insert(LastRaces[RaceData.RaceId], {
            ...
            BestLap = BLap,
            Holder = {
                [1] = acoreChar.firstname,
                [2] = acoreChar.lastname
            }
        })
    else
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('aphone:client:RaceNotify', src, 'Je hebt het WR van '..RaceData.RaceName..' verbroken met een tijd van: '..SecondsToClock(BLap)..'!')
        end
    else
        Races[RaceData.RaceId].Records = {
            Time = BLap,
        ...
        MySQL.update("UPDATE `lapraces` SET `records` = ? WHERE `raceid` = ?", { json.encode(Races[RaceData.RaceId].Records), RaceData.RaceId})
        TriggerClientEvent('aphone:client:RaceNotify', src, 'Je hebt het WR van '..RaceData.RaceName..' neergezet met een tijd van: '..SecondsToClock(BLap)..'!')
    end
    AvailableRaces[AvailableKey].RaceData = Races[RaceData.RaceId]
    TriggerClientEvent('laprace:client:PlayerFinishs', -1, RaceData.RaceId, PlayersFinished, acoreChar)
    ...
    TriggerClientEvent('aphone:client:UpdateLapraces', -1)
end)
...
RegisterServerEvent('laprace:server:CreateLapRace')
AddEventHandler('laprace:server:CreateLapRace', function(RaceName)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    
    if IsNameAvailable(RaceName) then
         TriggerClientEvent('laprace:client:StartRaceEditor', source, RaceName)
    else
        TriggerClientEvent('notification', source, 'There is already a race with this name.', 2)
    end
end)

ESX.RegisterServerCallback('laprace:server:GetRaces', function(source, cb)
    cb(AvailableRaces)
end)

ESX.RegisterServerCallback('laprace:server:GetListedRaces', function(source, cb)
    cb(Races)
end)

ESX.RegisterServerCallback('laprace:server:GetRacingData', function(source, cb, RaceId)
    cb(Races[RaceId])
end)

ESX.RegisterServerCallback('laprace:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(ESX.GetPlayerFromId(source).identifier))
end)

ESX.RegisterServerCallback('laprace:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
    cb(true)
end)
...
ESX.RegisterServerCallback('laprace:server:GetTrackData', function(source, cb, RaceId)
    MySQL.query("SELECT * FROM `users` WHERE `identifier` = ?", { Races[RaceId].Creator }, function(result)
...
RegisterServerEvent('laprace:server:JoinRace')
AddEventHandler('laprace:server:JoinRace', function(RaceData)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    local acoreChar = GetCharacterName(src)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('laprace:client:LeaveRace', src, Races[CurrentRace])
        else
    ...
    TriggerClientEvent('laprace:client:JoinRace', src, Races[RaceId], RaceData.Laps)
    TriggerClientEvent('aphone:client:UpdateLapraces', -1)
    ...
    if creatorsource ~= Player.source then
        TriggerClientEvent('aphone:client:RaceNotify', creatorsource, string.sub(acoreChar.firstname, 1, 1)..'. '..acoreChar.lastname..' is de race gejoined!')
    end
end)

RegisterServerEvent('laprace:server:LeaveRace')
AddEventHandler('laprace:server:LeaveRace', function(RaceData)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    local acoreChar = GetCharacterName(src)
    local RaceName
    ...
    local creatorsource = ESX.GetPlayerFromIdentifier(AvailableRaces[AvailableKey].SetupSteam).source
    if creatorsource ~= Player.source then
        TriggerClientEvent('aphone:client:RaceNotify', creatorsource, string.sub(acoreChar.firstname, 1, 1)..'. '..acoreChar.lastname..' is de race geleaved!')
    end
    local AmountOfRacers = 0
    for k, v in pairs(Races[RaceData.RaceId].Racers) do
        AmountOfRacers = AmountOfRacers + 1
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('notification', src, 'You were the only one in the race. The race is over.', 2)
        TriggerClientEvent('laprace:client:LeaveRace', src, Races[RaceId])
...
RegisterServerEvent('laprace:server:SetupRace')
AddEventHandler('laprace:server:SetupRace', function(RaceId, Laps)
                    ...
                    if Races[RaceId].Waiting then
                        ...
                        for cid,_ in pairs(Races[RaceId].Racers) do
                            ...
                            if RacerData ~= nil then
                                TriggerClientEvent('laprace:client:LeaveRace', RacerData.source, Races[RaceId])
                            end
                        end
                        table.remove(AvailableRaces, AvailableKey)
...
RegisterServerEvent('laprace:server:UpdateRaceState')
AddEventHandler('laprace:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterServerEvent('laprace:server:UpdateRacerData')
AddEventHandler('laprace:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    ...
    TriggerClientEvent('laprace:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterServerEvent('laprace:server:StartRace')
AddEventHandler('laprace:server:StartRace', function(RaceId)
    local src = source
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupSteam == MyPlayer.identifier then
            ...
            for identifier,_ in pairs(Races[RaceId].Racers) do
                ...
                if Player ~= nil then
                    TriggerClientEvent('laprace:client:RaceCountdown', Player.source)
                end
            end
            TriggerClientEvent('aphone:client:UpdateLapraces', -1)
        else
...
RegisterServerEvent('laprace:server:SaveRace')
AddEventHandler('laprace:server:SaveRace', function(RaceData)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    ...
    MySQL.insert("INSERT INTO `lapraces` (`name`, `checkpoints`, `creator`, `distance`, `raceid`) VALUES (?, ?, ?, ?, ?)", { RaceData.RaceName, json.encode(Checkpoints), Player.identifier, RaceData.RaceDistance, GenerateRaceId() })
end)
...
ESX.RegisterServerCallback('laprace:server:CanRaceSetup', function(source, cb)
