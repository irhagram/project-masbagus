                            ...
                            if dst <= 1 and IsControlJustReleased(0,38) then
                                ESX.TriggerServerCallback("utk_oh:startevent", function(output)
                                    ...
                                    elseif output ~= true then
                                        exports["midp-tasknotify"]:SendAlert("error", output)
                                    end
                                end, 1)
                            end
                        end
                                            ...
                                            if output then
                                                exports['midp-ui']:OpenHackingGame(function(output)
                                                    if output == true then 
                                                        ...
                                                        exports['midp-tasknotify']:SendAlert("success", UTK.text.hacked)
                                                        UTK.hackfinish = true
                                                    elseif output == false then
                                                        TriggerServerEvent("utk_oh:removeitem", "laptop_h")
                                                        exports['midp-tasknotify']:SendAlert("error", "HACKING GAGAL!")
                                                    end
                                                end)
                                            elseif not output then
                                                exports['midp-tasknotify']:SendAlert("error", "Anda Tidak Memiliki Laptop")
                                            end
                                        end, "laptop_h")
                                    end
                                end
                                                ...
                                                exports['midp-ui']:OpenHackingGame(function(output)
                                                    if output == true then 
                                                        ...
                                                        repeat
                                                            ...
                                                            count = count - 1
                                                        until count == 0
                                                        exports['midp-tasknotify']:SendAlert("success", UTK.text.used)
                                                        SpawnObj()
                                ...
                                if dst <= 2 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            exports["midp-tasknotify"]:SendAlert("error", "You don't have the ID Card.")
                                        end
                                        end, "id_card")
                                    end
                                end
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                exports['midp-tasknotify']:SendAlert("error", "Anda TIdak Memiliki Thermite")
                                            end
                                        end, "thermite")
                                    end
                                end
                                        ...
                                        if dst <= 1 and IsControlJustReleased(0, 38) then
                                            ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    self:Lockpick()
                                                elseif not output then
                                                    exports['midp-tasknotify']:SendAlert("error", "Anda tidak memiliki lockpick")
                                                end
                                            end, "lockpick")
                                        end
                                    end
                                            ...
                                            ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    if success then 
                                                        ...
                                                        exports['midp-tasknotify']:DoHudText('error', 'Gagal!') 
                                                        TriggerServerEvent("utk_oh:removeitem", "lockpick")
                                                    end
                                                elseif not output then
                                                    exports['midp-tasknotify']:SendAlert("error", "Anda tidak memiliki lockpick")
...
function UTK:Plant()
    ...
    Citizen.Wait(9000)
    exports['midp-tasknotify']:SendAlert("success", self.text.melted)
...
function UTK:Lockpick()
    ...
    Citizen.Wait(6500)
    exports['midp-tasknotify']:SendAlert("success", "Stage 1 "..self.text.stage)
    SetEntityCoords(ped, loc.x, loc.y, loc.z, 1, 0, 0, 1)
    SetEntityHeading(ped, loc.h)
    TaskPlayAnim(ped, animDict, "a_uncuff", 8.0, 8.0, 6000, 1, 1, 0, 0, 0)
    --[[ exports['progressBars']:startUI(6000, self.text.lockpick..": Stage 2") ]]
    Citizen.Wait(6500)
    exports['midp-tasknotify']:SendAlert("success", "Stage 2 "..self.text.stage)
    exports['midp-tasknotify']:SendAlert("success", self.text.unlocked)
    UTK.disableinput = false
    if self.currentpick == 0 then
        UTK.stage0break = true
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports['midp-tasknotify']:SendAlert("error", "Anda hanya punya waktu 300 Detik, sebelum gas beracun muncul!")
        UTK.starttimer = true
        UTK.vaulttime = 300
        Citizen.CreateThread(function()
            while true do
        ...
        ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            ...
            exports["midp-tasknotify"]:SendAlert("success", "Mencuri "..text)
        end)
    else
        exports['midp-tasknotify']:SendAlert("error", "Anda hanya punya waktu 300 Detik, sebelum gas beracun muncul!")
        UTK.starttimer = true
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        exports["midp-tasknotify"]:SendAlert("error", "Pintu bank akan tertutup 60 detik lagi!")
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                exports["midp-tasknotify"]:SendAlert("error", "Pintu brankas tertutup!")
                ...
                if UTK.grabber then
                    ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
                        text = "$"..ESX.Math.GroupDigits(result)
                        exports["midp-tasknotify"]:SendAlert("success", "You stole "..text)
                    end)
                end
                return
            end
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job.name == "police" then
        if toggle == 1 then
            exports["midp-tasknotify"]:SendAlert("infrom", "Terjadi perampokan di bank besar!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
