...
RegisterServerEvent("midp-spawn:LastLocation")
AddEventHandler('midp-spawn:LastLocation', function()
    local src = source
    local spawn = {}
    spawn = GetSpawnPos(src)

    if spawn == nil then
        TriggerClientEvent('midp-tasknotify:client:SendAlert', source, { type = 'error', text = 'Silahkan Pilih Bandara/Pelabuhan!'})
    else
        TriggerClientEvent("midp-spawn:LastLocationSpawn", src, spawn)
    end
end)

ESX.RegisterServerCallback("midp-spawn:showLastLocation", function(source, cb)
...
ESX.RegisterServerCallback('midp-spawn:getOwnedProperties', function(source, cb)
...
ESX.RegisterServerCallback('midp-spawn:getOwnedRumah', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
