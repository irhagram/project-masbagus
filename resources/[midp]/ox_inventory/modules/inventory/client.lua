...
if shared.target then
	...
	exports.ox_target:AddTargetModel(Inventory.Dumpsters, {
		options = {
			{
			...
			if client.hasGroup(shared.police) then
				if shared.target then
					if evidence.target then
						exports.ox_target:RemoveZone(evidence.target.name)
						exports.ox_target:AddBoxZone(evidence.target.name, evidence.target.loc, evidence.target.length or 0.5, evidence.target.width or 0.5,
						{
			...
			if not stash.groups or client.hasGroup(stash.groups) then
				if shared.target then
					if stash.target then
						exports.ox_target:RemoveZone(stash.name)
						exports.ox_target:AddBoxZone(stash.name, stash.target.loc, stash.target.length or 0.5, stash.target.width or 0.5,
						{
							name = stash.name,
