...
local mainMenuOptions, emoteMenuOptions, emoteMenuBindsOptions = {
    -- Main Menu
    {label = 'Emotes', icon = 'fa-solid fa-sign-language', values = {}, args = 'Emotes', close = false},
    {label = '>>  KATEGORI LAINNYA  <<', description = 'Buka Kategori Emotes', icon = 'fa-bars', args = 'animations_emote_menu'},
    {label = '>>  CARI (keywords)  <<', description = 'Cari Animasi', icon = 'fa-solid fa-magnifying-glass', args = 'animations_search'},
    ...
    -- Emote Menu
    {label = 'Prop Emotes', icon = 'fa-solid fa-cubes', values = {}, args = 'PropEmotes', close = false},
    {label = 'Dance Emotes', icon = 'fa-solid fa-music', values = {}, args = 'DanceEmotes', close = false},
    {label = 'Synchronized Emotes', icon = 'fa-solid fa-heartbeat', values = {}, args = 'SynchronizedEmotes', close = false},
    {label = 'Pose Emotes', icon = 'fa-solid fa-camera', values = {}, args = 'ConsumableEmotes', close = false},
    {label = 'Scenarios', icon = 'fa-solid fa-film', values = {}, args = 'Scenarios', close = false},    
...
function EmoteMenu.PlayRegisteredEmote(emote)
    ...
    if not registeredEmote then
        EmoteMenu.Notify('error', 'Emote tidak ditemukan')
        return
    end
...
function EmoteMenu.RequestSynchronizedEmote(senderData)
    ...
    if not targetId then
        EmoteMenu.Notify('error', 'Tidak ada orang disekitar!')
        return
    end
    ...
    TriggerServerEvent('midp-emotes:requestSynchronizedEmote', GetPlayerServerId(targetId), senderData, emote)
end
...
function EmoteMenu.Play(data, variation)
    ...
    if Config.EnableWeaponBlock and IsPedArmed(cache.ped, 7) then
        EmoteMenu.Notify('error', 'Tidak bisa menggunakan emote ketika bawa senjata!')
        return 
    end
    ...
    if not Config.AllowedInVehicles and isInVehicle then
        EmoteMenu.Notify('error', 'Tidak bisa menggunakan emote di kendaraan!')
        return
    end
    ...
    if data.Scenario then
        if isInVehicle then
            EmoteMenu.Notify('error', 'Tidak menggunakan emote di kendaraan!')
            return
        end
    ...
    if data.Options then
        ...
        if data.Options.Ptfx then
            ...
            TriggerServerEvent('midp-emotes:syncPtfx', data.Options.Ptfx.Asset, data.Options.Ptfx.Name, data.Options.Ptfx.Placement, data.Options.Ptfx.Color)
        else
        ...
        if propCount > 0 then
            ...
            for i = 1, propCount do
                ...
                if (i == 1) and (data.Options.Ptfx and data.Options.Ptfx.AttachToProp) then
                    TriggerServerEvent('midp-emotes:syncProp', ObjToNet(object))
                end
            end
        end
    end
...
function EmoteMenu.CancelAnimation()
    if EmoteMenu.IsPlayingAnimation then
        ...
        if EmoteMenu.OtherPlayer then
            TriggerServerEvent('midp-emotes:cancelSynchronizedEmote', EmoteMenu.OtherPlayer)
...
function EmoteMenu.Search(query)
    ...
    lib.registerMenu({
        ...
        if not _type then
            EmoteMenu.Notify('error', 'Itu bukan emote atau gaya berjalan yang valid')
            return
        end
        if _type == 'Walks' then
...
function EmoteMenu.OpenBindMenu()
    lib.registerMenu({
        ...
        if option == 'animations_create_bind' then
            local query = lib.inputDialog('Bind Emote', {'Command Nama Emote'})
            if not query then return end
            local _type, emote = EmoteMenu.GetEmoteByCommand(query[1])
            if not _type or (_type == 'Walks') then
                EmoteMenu.Notify('error', 'Itu bukan emote yang valid')
                return
            end
...
lib.registerMenu({
    ...
    title = 'DAILYLIFE EMOTES',
    position = Config.MenuPosition,
    options = mainMenuOptions,
}, function(selected, scrollIndex, option)
    if EmoteMenu.isActionsLimited then return end
    if option == 'animations_search' then
        local query = lib.inputDialog('Cari Emote', {'Emote:'})
        if not query then return end
...
exports["midp-core"]:registerKeyMapping('Emote Menu', Config.MenuOpenCommands[1], '')

RegisterCommand('+ecancel', function()
    EmoteMenu.CancelAnimation()
end)

RegisterCommand('+ndodok', function()
...
-- Events
RegisterNetEvent('midp-emotes:cancelAnimation', function()
    EmoteMenu.CancelAnimation()
end)

RegisterNetEvent('midp-emotes:closeMenu', function()
    EmoteMenu.CloseMenu()
end)

RegisterNetEvent('midp-emotes:play', function(emoteData, emoteVariant)
    EmoteMenu.Play(emoteData, emoteVariant)
end)

RegisterNetEvent('midp-emotes:playByCommand', function(emoteCommand, emoteVariant)
    EmoteMenu.PlayByCommand(emoteCommand, emoteVariant)
end)

RegisterNetEvent('midp-emotes:playRegisteredEmote', function(emoteName)
    EmoteMenu.PlayRegisteredEmote(emoteName)
end)

RegisterNetEvent('midp-emotes:registerEmote', function(emoteData)
    EmoteMenu.RegisterEmote(emoteData)
end)

RegisterNetEvent('midp-emotes:resetExpression', function()
    EmoteMenu.ResetExpression()
end)

RegisterNetEvent('midp-emotes:setExpression', function(expression)
    EmoteMenu.SetExpression(expression)
end)

RegisterNetEvent('midp-emotes:resetWalk', function()
    EmoteMenu.ResetWalk()
end)

RegisterNetEvent('midp-emotes:setWalk', function(walk)
    EmoteMenu.SetWalk(walk)
end)

RegisterNetEvent('midp-emotes:toggleLimitation', function(_state)
    EmoteMenu.ToggleLimitation(_state)
end)

RegisterNetEvent('midp-emotes:toggleMenu', function()
    EmoteMenu.ToggleMenu()
end)

local IsControlJustPressed = IsControlJustPressed

RegisterNetEvent('midp-emotes:synchronizedEmoteRequest', function(sender, senderData, targetData)
    ...
    while true do
        ...
        if IsControlJustPressed(0, 246) then
            lib.hideTextUI()
            TriggerServerEvent('midp-emotes:synchronizedEmoteResponse', sender, senderData, targetData)
            break
...
RegisterNetEvent('midp-emotes:targetStartSynchronizedEmote', function(sender, senderData, targetData)
...
RegisterNetEvent('midp-emotes:senderStartSynchronizedEmote', function(target, senderData)
...
RegisterNetEvent('midp-emotes:cancelSynchronizedEmote', function()
