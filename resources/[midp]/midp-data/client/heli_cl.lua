...
local polmav_hash2 = GetHashKey("polmav1")
local heli_hash3 = GetHashKey("b412")
...
function helicam()
	...
	if IsHeliHighEnough(heli) and IsPlayerInPolmav() then
...
Citizen.CreateThread(function()
    ...
    TriggerEvent("chat:removeSuggestion", "/+helicam")
    TriggerEvent("chat:removeSuggestion", "/-helicam")
    TriggerEvent("chat:removeSuggestion", "/+helirappel")
    TriggerEvent("chat:removeSuggestion", "/-helirappel")
end)
...
function IsPlayerInPolmav()
	...
	local isPolmav1 = IsVehicleModel(vehicle, polmav_hash2)
	local helihash3 = IsVehicleModel(vehicle, heli_hash3)
	if isPolmav or isPolmav1 or helihash3 then
		return true
	else
		return false
	end
