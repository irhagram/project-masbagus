...
function OpenBossMenu(society, close, options)
	options = options or {}
	local elements = {
		{unselectable = true, icon = "fas fa-user", title = 'Boss Menu'}
	}
	...
	ESX.TriggerServerCallback('esx_society:isBoss', function(isBoss)
		if isBoss then
			...
			if options.checkBal then
				elements[#elements+1] = {icon = "fas fa-wallet", title = 'Cek society balance', value = "check_society_balance"}
			end
			if options.withdraw then
				elements[#elements+1] = {icon = "fas fa-wallet", title = 'Ambil Uang Kantor', value = "withdraw_society_money"}
			end
			if options.deposit then
				elements[#elements+1] = {icon = "fas fa-wallet", title = 'Deposit Uang Kantor', value = "deposit_money"}
			end
			--[[if options.wash then
				elements[#elements+1] = {icon = "fas fa-wallet", title = 'Cuci Uang', value = "wash_money"}
			end]]
			if options.employees then
				elements[#elements+1] = {icon = "fas fa-users", title = 'Management Anggota', value = "manage_employees"}
			end
			--[[if options.salary then
				elements[#elements+1] = {icon = "fas fa-wallet", title = 'Management Gaji', value = "manage_salary"}
			end
			if options.grades then
				elements[#elements+1] = {icon = "fas fa-scroll", title = 'Management Pangkat', value = "manage_grades"}
			end]]
			...
			ESX.OpenContext("right", elements, function(menu,element)
				...
				elseif element.value == "withdraw_society_money" then
					local elements = {
						{unselectable = true, icon = "fas fa-wallet", title = 'Jumlah Ambil', description = "Withdraw money from the society account"},
				...
				elseif element.value == "deposit_money" then
					local elements = {
						{unselectable = true, icon = "fas fa-wallet", title = 'Jumlah', description = "Deposit some money into the society account"},
				...
				elseif element.value == "wash_money" then
					local elements = {
						{unselectable = true, icon = "fas fa-wallet", title = 'Jumlah', description = "Deposit some money into the money wash"},
...
function OpenManageEmployeesMenu(society)
	local elements = {
		{unselectable = true, icon = "fas fa-users", title = 'Manajemen Anggota'},
		{icon = "fas fa-users", title = 'List Anggota', value = "employee_list"},
		{icon = "fas fa-users", title = 'Rekrut', value = "recruit"}
	}
	ESX.OpenContext("right", elements, function(menu,element)
			...
			if element.value == "return" then
				...
				ESX.OpenContext("right", elements2, function(menu2,element2)
					...
					elseif element2.value == "fire" then
						ESX.ShowNotification('Memecat: ', employee.name)
...
function OpenRecruitMenu(society)
	ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
		local elements = {
			{unselectable = true, icon = "fas fa-user", title = 'Merekrut'}
		}
		...
		ESX.OpenContext("right", elements, function(menu,element)
			if element.value == "return" then
				...
				local elements2 = {
					...
					{icon = "fas fa-times", title = 'Tidak', value = "no"},
					{icon = "fas fa-check", title = 'Yaa', value = "yes"},
				}
				ESX.OpenContext("right", elements2, function(menu2,element2)
					if element2.value == "yes" then
						ESX.ShowNotification('Mengangkat: ', element.name)
...
function OpenPromoteMenu(society, employee)
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
		local elements = {
			{unselectable = true, icon = "fas fa-user", title = 'Mengangkat', employee.name}
		}
		...
		ESX.OpenContext("right", elements, function(menu,element)
			if element.value == "return" then
				...
				ESX.ShowNotification('Anda Mengangkat: ', employee.name, element.title)
...
function OpenManageSalaryMenu(society)
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
		local elements = {
			{unselectable = true, icon = "fas fa-wallet", title = 'Management Gaji'}
		}
		...
		for i=1, #job.grades, 1 do
			...
			elements[#elements+1] = {
				icon = "fas fa-wallet",
				title = ('%s - <span style="color:green;">%s</span>'):format(gradeLabel, 'money_generic', ESX.Math.GroupDigits(job.grades[i].salary)),
...
function OpenManageGradesMenu(society)
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
		local elements = {
			{unselectable = true, icon = "fas fa-wallet", title = 'Manajemen Pangkat'}
		}
