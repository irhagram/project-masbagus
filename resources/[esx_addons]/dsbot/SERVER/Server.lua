...
PerformHttpRequest(DiscordWebhookSystemInfos, function(Error, Content, Head) end, 'POST', json.encode({username = SystemName, content = ''}), { ['Content-Type'] = 'application/json' })

AddEventHandler('playerConnecting', function()
    local hexcok = GetPlayerIdentifier(source)
	--local tanggalWaktu = os.date("%x %X", os.time() + 7.5 * 60 * 60 )
	local dateLog = os.date('*t', os.time() + 7 * 60 * 60)
	--local date = os.date('*t')
	if dateLog.day < 10 then dateLog.day = '0' .. tostring(dateLog.day) end
	if dateLog.month < 10 then dateLog.month = '0' .. tostring(dateLog.month) end
	if dateLog.hour < 10 then dateLog.hour = '0' .. tostring(dateLog.hour) end
	if dateLog.min < 10 then dateLog.min = '0' .. tostring(dateLog.min) end
	if dateLog.sec < 10 then dateLog.sec = '0' .. tostring(dateLog.sec) end
	--TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookSystemInfos, SystemName, '`' .. tanggalWaktu .. '`\n```css\n' .. GetPlayerName(source) .. ' connecting\n```', SystemAvatar, false)
	TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookSystemInfos, SystemName, '`' .. dateLog.day .. '.' .. dateLog.month .. '.' .. dateLog.year .. ' - ' .. dateLog.hour .. ':' .. dateLog.min .. ':' .. dateLog.sec .. '`\n```diff\n ' .. GetPlayerName(source) .. ' [' .. hexcok ..  '] connecting\n```', SystemAvatar, false)
end)

AddEventHandler('playerDropped', function(Reason)
    local hexcok = GetPlayerIdentifier(source)
	--local tanggalWaktu = os.date("%x %X", os.time() + 7.5 * 60 * 60 )
	local dateLogKel = os.date('*t', os.time() + 7 * 60 * 60)
	--local date = os.date('*t')
	if dateLogKel.day < 10 then dateLogKel.day = '0' .. tostring(dateLogKel.day) end
	if dateLogKel.month < 10 then dateLogKel.month = '0' .. tostring(dateLogKel.month) end
	if dateLogKel.hour < 10 then dateLogKel.hour = '0' .. tostring(dateLogKel.hour) end
	if dateLogKel.min < 10 then dateLogKel.min = '0' .. tostring(dateLogKel.min) end
	if dateLogKel.sec < 10 then dateLogKel.sec = '0' .. tostring(dateLogKel.sec) end
	TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookSystemInfos, SystemName, '`' .. dateLogKel.day .. '.' .. dateLogKel.month .. '.' .. dateLogKel.year .. ' - ' .. dateLogKel.hour .. ':' .. dateLogKel.min .. ':' .. dateLogKel.sec .. '`\n```fix\n ' .. GetPlayerName(source) .. ' [' .. hexcok .. '] left (' .. Reason .. ')\n```', SystemAvatar, false)
...
AddEventHandler('DiscordBot:playerDied', function(Message, Weapon)
	--local date = os.date('*t')
	local date = os.date('*t', os.time() + 7 * 60 * 60)
	...
	--TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookKillinglogs, SystemName, '`' .. tanggalWaktu .. '`\n```' .. Message .. '```', SystemAvatar, false)
	TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookKillinglogs, SystemName, '`' .. date.day .. '.' .. date.month .. '.' .. date.year .. ' - ' .. date.hour .. ':' .. date.min .. ':' .. date.sec .. '`\n```' .. Message .. '```', SystemAvatar, false)
end)
