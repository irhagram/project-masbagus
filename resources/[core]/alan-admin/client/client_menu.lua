...
AddEventHandler('esx:onPlayerSpawn', function()
	...
	if spawnped then 
		Wait(5000)
		TriggerServerEvent("alan-admin:provideped", PlayerPedId()) 
	else
		--
	end
end)
    ...
    while true do
        ...
        elseif WarMenu.IsMenuOpened('utility') then ------------SELF MENU------------
            if WarMenu.Button("TP Marker") then
                TriggerEvent("alan-admin:tpm")
                TriggerServerEvent("alan-admin:wedos", 'TP Marker')
            elseif WarMenu.MenuButton('Vehicle Menu', 'vehicle')  then
            elseif WarMenu.Button("No Clip") then
                ExecuteCommand('noclip')
                TriggerServerEvent("alan-admin:wedos", 'Noclip')
            elseif WarMenu.Button("Refresh") then              
                TriggerEvent("alan-admin:RefreshPlayer", GetPlayerServerId(opt))
                TriggerServerEvent("alan-admin:wedos", 'Refresh Player')
            elseif WarMenu.Button("Invisible") then
                if invisible == true then 
                    invisible = false 
                    SetEntityVisible(GetPlayerPed(-1), false, 0)
                    TriggerServerEvent("alan-admin:wedos", 'Invisible = False')
                else
                    invisible = true 
                    SetEntityVisible(GetPlayerPed(-1), true, 0)
                    TriggerServerEvent("alan-admin:wedos", 'Invisible = True')             
                end
            ...
            elseif WarMenu.Button("Armor") then
                ...
                TriggerServerEvent("alan-admin:wedos", 'Armor')
            elseif WarMenu.Button("Godmode") then
                godmode = not godmode
                if godmode then
                    SetEntityInvincible(PlayerPedId(), true)
                    exports['alan-tasknotify']:SendAlert('success', 'GodMode ON')
                    TriggerServerEvent("alan-admin:wedos", 'GodMode = ON')             
                else
                    SetEntityInvincible(PlayerPedId(), false)    
                    exports['alan-tasknotify']:SendAlert('success', 'GodMode OFF')  
                    TriggerServerEvent("alan-admin:wedos", 'GodMode = OFF')    
                end    
            elseif WarMenu.Button("Suicide") then
                SetEntityHealth(PlayerPedId(), 0)
                TriggerServerEvent("alan-admin:wedos", 'Suicide')
            ...
            elseif WarMenu.Button("Clear Vehicle") then
                ShowTextEntry('Radius', "", function(result)
                    TriggerEvent('alan-admin:deleteVehicle', result)
                    TriggerServerEvent("alan-admin:wedos", 'Clear Vehicle Dengan Radius:' ..result)
                end)
            elseif WarMenu.Button("Clear Object") then
                ShowTextEntry('Radius', "", function(result)
                    TriggerEvent('alan-admin:deleteObj', result)
                    TriggerServerEvent("alan-admin:wedos", 'Clear Object Dengan Radius:' ..result)
                end)
            elseif WarMenu.Button("Clear Peds") then
                TriggerEvent('alan-admin:deletePeds')   
                TriggerServerEvent("alan-admin:wedos", 'Delete Peds')
            elseif WarMenu.Button("Change Skin") then
                ESX.TriggerServerCallback("alan-admin:getadminRank", function(playerRank)
                    if playerRank == "superadmin" then
                        ShowTextEntry('Skin Model', "", function(result)
                            if result then
                                TriggerEvent('alan-admin:changeSkin', result)
                                TriggerServerEvent("alan-admin:wedos", " Menggunakan Fitur Change Skin")
                            end
            ...
            elseif WarMenu.Button("Show Blip") then
                ...
                if showblips == true then 
                    showblips = false 
                    TriggerServerEvent("alan-admin:wedos", 'Show blips = False')
                else
                    showblips = true 
                    TriggerServerEvent("alan-admin:wedos", 'Show blips = Off')
                end
            end
        end)
            ...
            elseif WarMenu.CheckBox("Coords",showCoords,function(enabled)showCoords = enabled end) then
                TriggerServerEvent("alan-admin:wedos", 'Show Coords')
            elseif WarMenu.CheckBox("Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end) then
                ESX.TriggerServerCallback("alan-admin:getadminRank", function(playerRank)
                if playerRank == "superadmin" then
                TriggerServerEvent("alan-admin:wedos", 'Infinite Stamina')
                end
            end)
            --[[elseif WarMenu.CheckBox("Infinite Ammo",InfAmmo,function(enabled)InfAmmo = enabled SetPedInfiniteAmmo(PlayerPedId(), InfAmmo)end) then
                ESX.TriggerServerCallback("alan-admin:getadminRank", function(playerRank)
                    if playerRank == "superadmin" then
                    TriggerServerEvent("alan-admin:wedos", 'Infinite Ammo')
                    end
                end)]]
        ...
        elseif WarMenu.IsMenuOpened('adminutility') then ------------ADMIN SELF MENU------------
            ...
            elseif WarMenu.Button("Max Performace Vehicle") then              
                ...
                TriggerServerEvent("alan-admin:wedos", 'Max Performace Veh')
            elseif WarMenu.Button("Invisible") then
                if invisible == true then 
                    invisible = false 
                    SetEntityVisible(GetPlayerPed(-1), false, 0)
        ...
        elseif WarMenu.IsMenuOpened("spawnobj") then ------------SPAWN OBJECT MENU------------
            ...
            if WarMenu.Button("Spawn Object: " .. (message2 and message2 or "")) then
                if message1 == '' then 
                    exports['alan-tasknotify']:SendAlert('error', 'Input yang bener!', 10000)
                else
        ...
        elseif WarMenu.IsMenuOpened("deletevehicle") then ------------DELETE VEHICLE MENU------------
            ...
            if WarMenu.Button("Delete Vehicle: " .. (message2 and message2 or "")) then
                if message1 == '' then 
                    ...
                    TriggerServerEvent("alan-admin:dv", message1)
                end
            end
        ...
        elseif WarMenu.IsMenuOpened("troll") then ------------TROLL MENU------------
            ...
            if WarMenu.Button("Spawn FBI") then
                ...
                TriggerServerEvent("alan-admin:wedos", 'Troll Player (FBI)')
            elseif WarMenu.Button("Spawn Macan") then
                ExecuteCommand('trollmacan '..selectedplayer)
                TriggerServerEvent("alan-admin:wedos", 'Troll Player (MACAN)')
            end
        ...
        elseif WarMenu.IsMenuOpened("givemoney") then ------------GIVEMONEY MENU------------
            ...
            if WarMenu.Button("Set Money: " .. (message4 and message4 or "")) then
                ...
                TriggerServerEvent("alan-admin:beriuangnyadongg", 'SETMONEY SEBANYAK >> ' .. message2 .. " DENGAN ALASAN >> " .. message3)
            end
        ...
        elseif WarMenu.IsMenuOpened("bannedmenu") then ------------BANNED ONLINE MENU------------
            ...
            if WarMenu.Button("Banned Player: " .. (message3 and message3 or "")) then
                if message1 == '' or message2 == '' then 
                    ...
                    --TriggerServerEvent('alan-admin:banplayeron', selectedplayer, message2, message1)
                    WarMenu.CloseMenu()
                end
            end
        ...
        elseif WarMenu.IsMenuOpened("bannedmenuoff") then ------------BANNED OFFLINE MENU------------
            ...
            if WarMenu.Button("Banned Player: " .. (message4 and message4 or "")) then
                if message1 == '' or message2 == '' or message3 == '' then 
                    ...
                    --TriggerServerEvent('alan-admin:banPlayeroff',message1, message2,message3)
                    WarMenu.CloseMenu()
                end
            end
        ...
        elseif WarMenu.IsMenuOpened("awarningp") then ------------ WARNING PLAYER ------------
            ...
            if WarMenu.Button("Kirim : " .. (message4 and message4 or "")) then
                if message1 == '' or message3 == '' then 
                    ...
                    TriggerServerEvent('alan-admin:inputwarning', message1, message3)
                    TriggerServerEvent("alan-admin:wedos", 'Memberikan Warning = ' .. message1 .. ' Alasan ' ..message3)
                    WarMenu.CloseMenu()
                end
            end
        ...
        elseif WarMenu.IsMenuOpened("gantiplat") then
            ...
            if IsPedInAnyVehicle(playerPed, true) then
                ...
                if WarMenu.Button("New plate: " .. (platetext3 and platetext3 or "0")) then
                    ...
                    ESX.TriggerServerCallback('alan-admin:update', function( cb )
                        if cb == 'found' then
                          SetVehicleNumberPlateText(vehicle, platetext1)

                        elseif cb == 'error' then
                            exports['alan-tasknotify']:SendAlert('error', 'Tidak bisa digunakan')
                        end
        ...
        elseif WarMenu.IsMenuOpened("spawnveh") then
            ...
            if WarMenu.Button("Spawn Vehicle: " .. (spawnveh and spawnveh or "0")) then
                ...
                if namemodel == 'NULL' then
                    exports['alan-tasknotify']:SendAlert('error', 'Kendaraan tidak terdaftar')
                else
        ...
        elseif WarMenu.IsMenuOpened("changeskin") then ------------CHANGE SKIN MENU------------
            ...
            if WarMenu.Button("Change Skin: " .. (message1 and message1 or "")) then
                TriggerEvent('alan-admin:ChangeSkin', message1)
            end
        ...
        elseif WarMenu.IsMenuOpened("aallplayer") then  
            if WarMenu.Button("Revive All") then
                ...
                TriggerServerEvent("alan-admin:wedos", 'Revive All')
            end
            if WarMenu.Button("Kick All") then
                TriggerServerEvent("alan-admin:kickall")
                TriggerServerEvent("alan-admin:wedos", 'Kick All')
            end
        ...
        elseif WarMenu.IsMenuOpened('playermanage') then ------------PLAYER LIST MENU------------
            if WarMenu.Button("Spectate") then
                if selectedplayer ~= GetPlayerServerId(PlayerId()) then
                    ...
                    if spectate then
                        ...
                        ESX.TriggerServerCallback('alan-admin:getTargetCoord', function(coord)
                            if coord ~= nil then
                                DoScreenFadeOut(200)
                                Citizen.Wait(350)
            ...
            elseif WarMenu.Button("Clone") then
                ESX.TriggerServerCallback("alan-admin:getadminRank", function(playerRank)
                    if playerRank == "superadmin" then
                TriggerEvent('alan-admin:ClonePed', GetPlayerPed(GetPlayerFromServerId(selectedplayer)))
            --     TriggerServerEvent("alan-admin:wedos", 'Clone Player')
                    end
                end)
            elseif WarMenu.Button("Frozen") then
                ExecuteCommand('freeze '..selectedplayer)
                TriggerServerEvent("alan-admin:wedos", 'Freeze')
            elseif WarMenu.Button("Open Inventory") then
                ExecuteCommand('viewinv '..selectedplayer)
                TriggerServerEvent("alan-admin:wedos", 'OpenInventory')
            elseif WarMenu.Button("Slay") then
                ESX.TriggerServerCallback("alan-admin:getadminRank", function(playerRank)
                    if playerRank == "superadmin" then
                ExecuteCommand('kill '..selectedplayer)
                TriggerServerEvent("alan-admin:wedos", 'Kill Player')
                    end
                end)
            ...
            elseif WarMenu.Button("Kick Player") then
                ShowTextEntry('Reason', "", function(reason)
                    ...
                    TriggerServerEvent("alan-admin:kickPlayer", selectedplayer, reason) 
                end)
            ...
            --elseif WarMenu.Button("Troll Menu") then
              --  ExecuteCommand('troll '..selectedplayer)
                --TriggerServerEvent("alan-admin:wedos", 'Troll Player')
            end
        ...
        elseif WarMenu.IsMenuOpened('adminplayermanage') then ------------ADMIN PLAYER LIST MENU------------
            ...
            elseif WarMenu.Button("Delete Vehicle") then
                ExecuteCommand('dv2 '..selectedplayer)
                TriggerServerEvent("alan-admin:wedos", 'DV Player Vehicle')
            elseif WarMenu.Button("Give money") then
				WarMenu.OpenMenu("givemoney")
            elseif WarMenu.Button("Clear Inventory") then
                ShowTextEntry('Y or N', "", function(confirm)
        ...
        elseif WarMenu.IsMenuOpened("recmenu") then
            if WarMenu.Button("Start Recording") then
                if IsRecording() then 
                    exports['alan-tasknotify']:SendAlert('error', 'Recording sedang berlangsung!', 10000)
                else
                    StartRecording(1)
                end
            end
            ...
            if WarMenu.Button("Stop Recording") then
                if not IsRecording() then 
                    exports['alan-tasknotify']:SendAlert('error', 'Recording tidak berlangsung!', 10000)
                else
...
function refreshPlayers()
    ESX.TriggerServerCallback('alan-admin:getPlayers', function(alantesplayer)
...
RegisterNetEvent('alan-admin:ChangeSkin')
AddEventHandler('alan-admin:ChangeSkin', function(model)
...
RegisterNetEvent("alan-admin:ClonePed")
AddEventHandler('alan-admin:ClonePed', function(target)
    local ped = target
...
RegisterNetEvent('alan-admin:deleteObj')
AddEventHandler('alan-admin:deleteObj', function(radius)
...
RegisterNetEvent('alan-admin:deletePeds')
AddEventHandler('alan-admin:deletePeds', function(radius)
    PedStatus = 0
...
RegisterNetEvent('alan-admin:deleteVehicle')
AddEventHandler('alan-admin:deleteVehicle', function(radius)
    radius = tonumber(radius)
    if radius == 0 then 
...
RegisterNetEvent('alan-admin:deleteVehiclee')
AddEventHandler('alan-admin:deleteVehiclee', function(x, y, z)
...
RegisterNetEvent('alan-admin:teleportUser')
AddEventHandler('alan-admin:teleportUser', function(x, y, z)
    local playerPed = PlayerPedId()
	SetEntityCoords(playerPed, x, y, z)
end)

RegisterNetEvent('alan-admin:RefreshPlayer')
AddEventHandler('alan-admin:RefreshPlayer', function()
    ClearPedTasksImmediately(PlayerPedId())
end)

RegisterNetEvent('alan-admin:spawnmacan')
AddEventHandler('alan-admin:spawnmacan', function()
...
RegisterNetEvent('alan-admin:spawnswat')
AddEventHandler('alan-admin:spawnswat', function()
...
RegisterNetEvent('alan-admin:changepedmenu', function()
	local input = lib.inputDialog('PED CHANGER', {'ID PLAYER:', 'NAMA PED:'})

	if not input then return end
	local idply = tonumber(input[1])
	local namaped = input[2]
	TriggerServerEvent('alan-admin:changeped', idply, namaped)
end)

RegisterCommand('gantiped', function()
	ESX.TriggerServerCallback("alan-admin:getadminRank", function(playerRank)
		if playerRank == "superadmin" then
			TriggerEvent("alan-admin:changepedmenu")
		else
			exports["alan-tasknotify"]:SendAlert("error", "Anda SIAPA YA?")
		end
	end)
end)
...
RegisterCommand("deleteped", function(source, args, raw)
	ESX.TriggerServerCallback("alan-admin:getadminRank", function(playerRank)
		if playerRank == "superadmin" then
			TriggerServerEvent("alan-admin:deleteped", args[1])
		else
			exports["alan-tasknotify"]:SendAlert("error", "Anda SIAPA YA?")
		end
	end)
...
RegisterNetEvent('alan-admin:setPed')
AddEventHandler('alan-admin:setPed', function(modelpeda)
...
RegisterNetEvent("alan-admin:loadSkin")
AddEventHandler("alan-admin:loadSkin", function()
