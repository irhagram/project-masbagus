        ...
        if isWhitelisted(source) then
            if args ~= nil then
				...
				if tonumber(p) and tonumber(l) then
					...
					TriggerClientEvent('midp-tasknotify:client:SendAlert', source, { type = 'success', text = 'Kode dibuat, silahkan cek log discord!'})
			...
			TriggerClientEvent('midp-tasknotify:client:SendAlert', source, { type = 'error', text = 'Anda SIAPA?'})
		end
    end
end)
...
RegisterCommand('tukarcode', function(source, args)
	if args[1] == nil then 
		TriggerClientEvent('midp-tasknotify:client:SendAlert', source, { type = 'error', text = 'Silahkan input kode yang di beri admin!'})
		return 
	end			
...
function set_vip(source,id)	
	...
	if result[1] ~= nil then
		...
		if not canjeado then
			...
			TriggerClientEvent('midp-tasknotify:client:SendAlert', source, { type = 'error', text = 'Kode sudah di tukar!'})
		end
	else
		TriggerClientEvent('midp-tasknotify:client:SendAlert', source, { type = 'error', text = 'Kode tidak valid!'})
	end
end
...
function add_month(source,mes,level)
	...
	MySQL.Async.fetchAll("SELECT identifier, level as level, DATE_FORMAT(expiration,'%Y-%m-%d') as expiration FROM daily_vip WHERE identifier = @identifier", {
		...
		if result[1] ~= nil then
			...
			TriggerClientEvent('midp-tasknotify:client:SendAlert', source, { type = 'inform', text = 'Kode berhasil ditukar'})
			update(source)
		else
