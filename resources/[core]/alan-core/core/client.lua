RegisterNetEvent('dl-rent:Show', function()
    ESX.TriggerServerCallback('alan-core:cekJob', function(jumlah)
        if jumlah < 1 then
          TriggerEvent('alan-core:sewaspd')
        else
          exports['alan-tasknotify']:DoHudText('error', 'Taxi sedang online, silahkan hubungi taxi!')
        end
    end, 'taxi')
end)
...
RegisterNetEvent('dl-rent:ShowLah', function()
    ESX.TriggerServerCallback('alan-core:cekJob', function(jumlah)
        if jumlah < 1 then
          TriggerEvent('alan-core:sewaVehLah')
        else
          exports['alan-tasknotify']:DoHudText('error', 'Taxi sedang online, silahkan hubungi taxi!')
        end
    end, 'taxi')
...
RegisterNetEvent('alan-core:CraftArmor', function()
    ESX.TriggerServerCallback('alan-core:cekJob', function(jumlah)
        if jumlah > 2 then
...
RegisterNetEvent('alan-core:CraftMachine', function()
    ESX.TriggerServerCallback('alan-core:cekJob', function(jumlah)
        if jumlah > 2 then
...
RegisterNetEvent('alan-core:CraftRampok', function()
    ESX.TriggerServerCallback('alan-core:cekJob', function(jumlah)
        if jumlah > 2 then
...
RegisterNetEvent('alan-core:CraftPluru', function()
    ESX.TriggerServerCallback('alan-core:cekJob', function(jumlah)
        if jumlah > 2 then
...
RegisterNetEvent('alan-core:CraftBedil', function()
  ESX.TriggerServerCallback('alan-core:cekJob', function(jumlah)
      if jumlah > 2 then
...
RegisterNetEvent('alan-core:sewaspd')
AddEventHandler('alan-core:sewaspd', function()
...
RegisterNetEvent('alan-core:sewaVehLah')
AddEventHandler('alan-core:sewaVehLah', function()
  TriggerEvent('alan-context:sendMenu', {
    {
      id = 1,
      header = "DAILYLIFE ROLEPLAY",
...
RegisterNetEvent('alan-core:masakPedagang')
AddEventHandler('alan-core:masakPedagang', function()
  TriggerEvent('alan-context:sendMenu', {
    {
      id = 1,
      header = "Menu Masak",
...
RegisterNetEvent('alan-core:spawnVeh')
AddEventHandler('alan-core:spawnVeh', function(data)
    ...
    ESX.Game.SpawnVehicle(data.type, {x = -1039.1832275391, y = -2727.9150390625, z = 20.038572311401}, 319.72, function(callback_vehicle)
        ...
        TriggerServerEvent('alan-core:bayardong', price)
    end)
end)

RegisterNetEvent('alan-core:spawnVehLah')
AddEventHandler('alan-core:spawnVehLah', function(data)
    local price = data.price
...
RegisterNetEvent('alan-core:alanMulai')
AddEventHandler('alan-core:alanMulai', function(itemId)
	  TriggerServerEvent('alan-core:cekKebutuhan', itemId.type)
end)

RegisterNetEvent('alan-core:mulaiBuat')
AddEventHandler('alan-core:mulaiBuat', function(item)
    isCrafting = true
    MasakHayuk(item)
end)
  ...
  if not sibuk then
      ...
      exports.ox_inventory:Progress({
        ...
        anim = { dict = 'mini@repair', clip = 'fixing_a_ped' },
          ...
          if not cancel then
              isCrafting = false
              TriggerServerEvent('alan-core:giveCraftedItem', item)
              sibuk = false
          else
              sibuk = false
