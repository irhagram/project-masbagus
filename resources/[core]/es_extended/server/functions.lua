...
Core.SavePlayer = function(xPlayer, cb)
	MySQL.prepare("UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `status` = ? WHERE `identifier` = ?", {{
...
ESX.UseItem = function(source, item, data)
	...
	if itemCallback then
		local success, result = pcall(itemCallback, source, item, data)
		if not success then
