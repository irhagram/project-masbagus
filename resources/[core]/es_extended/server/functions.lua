...
function ESX.RandomStr(length)
	if length <= 0 then return '' end
	return ESX.RandomStr(length - 1) .. StringCharset[math.random(1, #StringCharset)]
end

function ESX.RandomInt(length)
	if length <= 0 then return '' end
	return ESX.RandomInt(length - 1) .. NumberCharset[math.random(1, #NumberCharset)]
end

function ESX.CreateCustomID()
	local fnd = true
	local cid = nil
	while fnd do
		cid = ''..tostring(ESX.RandomStr(3) .. ESX.RandomInt(5)):upper()..''
		print(cid)
		local result = MySQL.Async.fetchAll('SELECT 1 FROM users WHERE cid = @cid', {
			['@cid'] = cid
		})
		if result == nil then
			fnd = false
