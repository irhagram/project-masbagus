...
ESX.Game.GetVehicleProperties = function(vehicle)
	if DoesEntityExist(vehicle) then
		...
		for i = 1, 7 do
			res = IsVehicleTyreBurst(vehicle, i, false)
			if res ~= nil then
				tyresburst[#tyresburst+1] = res
				if res == false then
					res = IsVehicleTyreBurst(vehicle, i, true)
					tyresburst[#tyresburst] = res
				end
			else
				tyresburst[#tyresburst+1] = false
			end
		end
	
		for i = 0, 5 do
			res = IsVehicleDoorDamaged(vehicle, i)
			if res then
				doors[#doors+1] = res
			else
				doors[#doors+1] = false
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	if DoesEntityExist(vehicle) then
		...
		if props.doors ~= nil then
			for doorsIndex = 0, 5, 1 do
				if (props.doors[doorsIndex] ~= false) then
					SetVehicleDoorBroken(vehicle, doorsIndex - 1, true)
				end
			end
		end
	
		if props.tyresburst ~= nil then
			for tyreIndex = 1, 7, 1 do
				if (props.tyresburst[tyreIndex] ~= false) then
					SetVehicleTyreBurst(vehicle, tyreIndex, true, 1000)
				end
			end
		end
