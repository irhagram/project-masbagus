...
Citizen.CreateThread(function()
    RegisterServerEvent("midp-admin:banPlayeron")
    AddEventHandler('midp-admin:banPlayeron', function(playerId,reason,expires)
...
RegisterServerEvent("midp-admin:kickPlayer")
AddEventHandler('midp-admin:kickPlayer', function(playerId,reason)
    local sourceXPlayer = ESX.GetPlayerFromId(source)
    local xPlayer = ESX.GetPlayerFromId(source)
    if havePermission(xPlayer) then
        DropPlayer(playerId,reason .. " oleh : " ..sourceXPlayer.name)
    else
        print(('midp-admin: %s attempted to kick player!'):format(GetPlayerIdentifiers(source)[1]))
    end
end)

RegisterServerEvent("midp-admin:dv")
AddEventHandler("midp-admin:dv", function(radius)
    local xPlayer = ESX.GetPlayerFromId(source)
    if havePermission(xPlayer) then
        TriggerClientEvent('midp-admin:deleteVehicle', source, radius)
    else
        print(('midp-admin: %s attempted to delete vehicle!'):format(GetPlayerIdentifiers(source)[1]))
    end
end)

ESX.RegisterServerCallback('midp-admin:update', function(source, cb, oldP, newP)
...
ESX.RegisterServerCallback('midp-admin:getTargetCoord', function(source, cb, target)
    local player = GetPlayerPed(target)
    local coord = GetEntityCoords(player)
    cb(coord)
end)

ESX.RegisterServerCallback("midp-admin:getadminRank", function(source, cb)
...
ESX.RegisterServerCallback('midp-admin:getPlayers', function(source, cb)
    local l = {}
...
RegisterServerEvent("midp-admin:inputwarning")
AddEventHandler("midp-admin:inputwarning", function(message1, message3)
...
RegisterServerEvent('midp-admin:changeped', function(player, ped)
    local args1 = player
    local modelpeda = ped
    ...
    local xIgrac = ESX.GetPlayerFromId(args1)
        if xIgrac then
            ...
            function(result)
                ...
                function(result)
                    TriggerClientEvent('midp-admin:setPed', xIgrac.source, modelpeda)
                    TriggerClientEvent('midp-tasknotify:client:SendAlert', xAdmin.source, { type = 'inform', text = 'Berhasil mengubah ped & disimpan ke database!'})
                    --discordLog(GetPlayerName(xAdmin.source)..' mengubah skin untuk '..GetPlayerName(player)..' dengan skin '..modelpeda, 'E-ADMIN v1.1 © 2022')
                end)
                else
                TriggerClientEvent('midp-tasknotify:client:SendAlert', xAdmin.source, { type = 'error', text = 'Pemain yang disebutkan sudah memiliki ped di database'})
                end
            end)
        else
            TriggerClientEvent('midp-tasknotify:client:SendAlert', xAdmin.source, { type = 'error', text = 'Player tidak online'})
        end
end)

RegisterServerEvent('midp-admin:deleteped', function(player)
    local args1 = player
    local xIgrac = ESX.GetPlayerFromId(args1)
    local xAdmin = ESX.GetPlayerFromId(source)
    local grupa = xAdmin.getGroup()
    if xIgrac then
        ...
        function(result)
            if result == nil then
            TriggerClientEvent('midp-tasknotify:client:SendAlert', xAdmin.source, { type = 'error', text = 'Pemain Anda tidak memiliki ped di database'})
            else
            MySQL.Async.fetchAll("DELETE FROM daily_ped WHERE identifier = @identifier",
            {["@identifier"] = xIgrac.identifier},
            function(result)
            TriggerClientEvent('midp-admin:setPed', xIgrac.source, 'mp_m_freemode_01')
            TriggerClientEvent('midp-admin:loadSkin', xIgrac.source)
                TriggerClientEvent('midp-tasknotify:client:SendAlert', xAdmin.source, { type = 'success', text = 'Ped berhasil dihapus dari database'})
...
RegisterNetEvent('midp-admin:provideped')
AddEventHandler('midp-admin:provideped', function(igrac)
local xPlayer = ESX.GetPlayerFromId(source)
    MySQL.Async.fetchScalar("SELECT * FROM daily_ped WHERE identifier = @identifier",
    ...
    function(result)
        ...
        {["@identifier"] = xPlayer.identifier},
        function(result)
            TriggerClientEvent('midp-admin:setPed', xPlayer.source, result[1].hash)
        end)
        end
    end)
end)

RegisterServerEvent("midp-admin:wedos")
AddEventHandler("midp-admin:wedos", function(option)
...
RegisterServerEvent("midp-admin:beriuangnyadongg")
AddEventHandler("midp-admin:beriuangnyadongg", function(option)
	local date = os.date('*t')
    local _source = source
    local identifier = GetPlayerIdentifier(source)
