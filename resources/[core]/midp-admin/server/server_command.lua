	  	...
	  	if havePermission(xPlayer) then
	    	if args[1] and tonumber(args[1]) then
	      		...
	      		if xTarget then
	        		...
	        		TriggerClientEvent('midp-tasknotify:client:SendAlert', source, { type = 'error', text = 'Berhasil'})
	      		else
	        		TriggerClientEvent('midp-tasknotify:client:SendAlert', source, { type = 'error', text = 'Berhasil'})
	      		end
	    	else
				TriggerClientEvent('midp-tasknotify:client:SendAlert', source, { type = 'error', text = 'Input yang bener!'})
	    	end
	  	end
	end
end, false)
...
RegisterCommand("noclip", function(source, args, rawCommand)	-- /goback will teleport you back where you was befor /goto
	if source ~= 0 then
	  	local xPlayer = ESX.GetPlayerFromId(source)
	  	if havePermission(xPlayer) then
	    	TriggerClientEvent("midp-admin:noclip", xPlayer.source)
	  	end
	end
end, false)
---------- Noclip2 --------
		...
		if havePermission(xPlayer) then
			if args[1] and tonumber(args[1]) then
				local targetId = tonumber(args[1])
      			local xTarget = ESX.GetPlayerFromId(targetId)
      			if xTarget then
					TriggerClientEvent("midp-admin:noclip", xTarget.source)
      			else
		...
		if havePermission(xPlayer) then
			if args[1] and tonumber(args[1]) then
				local targetId = tonumber(args[1])
      			local xTarget = ESX.GetPlayerFromId(targetId)
      			if xTarget then
					TriggerClientEvent("midp-admin:teleportUser", xTarget.source)
      			else
        			TriggerClientEvent("chatMessage", xPlayer.source, _U('not_online', 'KILL'))
      			end
    		else
		...
		if havePermission(xPlayer) then
			if args[1] and tonumber(args[1]) then
				local targetId = tonumber(args[1])
      			local xTarget = ESX.GetPlayerFromId(targetId)
      			if xTarget then
					TriggerClientEvent("midp-admin:RefreshPlayer", xTarget.source)
      			else
        			TriggerClientEvent("chatMessage", xPlayer.source, _U('not_online', 'KILL'))
      			end
    		else
	...
	if cm[1] == "/reply" or cm[1] == "/rr" or cm[1] == "/r" then
		...
		if tablelength(cm) > 1 then
		    ...
		    if playerGroup ~= 'user' or playerGroup ~= 'superadmin' or playerGroup ~= 'admin' then
			    ...
			    TriggerClientEvent('midp-tasknotify:client:SendAlert', source, { type = 'error', text = 'Tidak ada aksess'})
			end
		end
	end	
end)
		...
		if havePermission(xPlayer) then
			if args[1] and tonumber(args[1]) then
				local targetId = tonumber(args[1])
      			local xTarget = ESX.GetPlayerFromId(targetId)
      			if xTarget then
					--TriggerClientEvent("midp-admin:spawnmacan", xTarget.source)
					TriggerClientEvent("midp-admin:spawnswat", xTarget.source)
      			else
        			TriggerClientEvent("chatMessage", xPlayer.source, _U('not_online', 'KILL'))
      			end
    		else
		...
		if havePermission(xPlayer) then
			if args[1] and tonumber(args[1]) then
				local targetId = tonumber(args[1])
      			local xTarget = ESX.GetPlayerFromId(targetId)
      			if xTarget then
					TriggerClientEvent("midp-admin:deleteVehiclee", xTarget.source)
      			else
        			TriggerClientEvent("chatMessage", xPlayer.source, _U('not_online', 'KILL'))
      			end
    		else
		...
		if havePermission(xPlayer) then
			if args[1] and tonumber(args[1]) then
				local targetId = tonumber(args[1])
      			local xTarget = ESX.GetPlayerFromId(targetId)
      			if xTarget then
					TriggerClientEvent("midp-admin:killPlayer", xTarget.source)
      			else
        			TriggerClientEvent("chatMessage", xPlayer.source, _U('not_online', 'KILL'))
      			end
    		else
		...
		if havePermission(xPlayer) then
			if args[1] and tonumber(args[1]) then
				local targetId = tonumber(args[1])
      			local xTarget = ESX.GetPlayerFromId(targetId)
      			if xTarget then
					TriggerClientEvent("midp-admin:slapplayer", xTarget.source)
      			else
        			TriggerClientEvent("chatMessage", xPlayer.source, _U('not_online', 'KILL'))
      			end
    		else
		...
		if havePermission(xPlayer) then
			if args[1] and tonumber(args[1]) then
				local targetId = tonumber(args[1])
      			local xTarget = ESX.GetPlayerFromId(targetId)
      			if xTarget then
					TriggerClientEvent("midp-admin:flyplayer", xTarget.source)
      			else
        			TriggerClientEvent("chatMessage", xPlayer.source, _U('not_online', 'KILL'))
      			end
    		else
		...
		if havePermission(xPlayer) then
			if args[1] and tonumber(args[1]) then
				local targetId = tonumber(args[1])
      			local xTarget = ESX.GetPlayerFromId(targetId)
      			if xTarget then
					TriggerClientEvent("midp-admin:freezeplayer", xTarget.source)
      			else
        			TriggerClientEvent("chatMessage", xPlayer.source, _U('not_online', 'KILL'))
      			end
    		else
...
RegisterCommand("spawnobj", function(source, args, rawCommand)	-- /kill [ID]
	local xPlayer = ESX.GetPlayerFromId(source)
	if havePermission(xPlayer) then
		TriggerClientEvent("midp-admin:spawnobj",source ,args[1])
	end
end, false)
---------- reviveall ----------
...
RegisterServerEvent("midp-admin:kickall")
AddEventHandler('midp-admin:kickall', function()
	kickPl()
end)
